...
uchar mfd_weapon_expose_projectile(MFD *m, weapon_slot *ws, ubyte control) {
    ...
    if ((control & MFD_EXPOSE_FULL) || (ws->ammo == 0))
        ...
        RedrawAmmoFlag = TRUE;
        MFD_Access(m->id, MFDAmmo) = ws->ammo;
    }

    // Redraw ammo buttons if neccessary
    if (RedrawAmmoFlag)
        mfd_weapon_draw_ammo_buttons(num_ammo_buttons, ammo_subclass, ammo_types, ws->ammo_type, ws->ammo);
...
ubyte temp_levels[TEMPR_WIDTH] = {1, 2, 2, 3, 3, 4, 5, 6, 7, 9, 11, 13, 16};
...
void mfd_weapon_draw_beam_status_bar(int amt, int setting, uchar does_overload) {
    ...
    ss_box(MfdBeamStatusRect.ul.x - 1, MfdBeamStatusRect.ul.y - 1, MfdBeamStatusRect.lr.x + 1,
    ...
    mfd_add_rect(MfdBeamStatusRect.ul.x, MfdBeamStatusRect.ul.y, MfdBeamStatusRect.lr.x, MfdBeamStatusRect.lr.y);
    return;
}
...
static ubyte cat2active[MFD_INV_CATEGORIES] = {
    NULL_ACTIVE,   ACTIVE_DRUG,    ACTIVE_HARDWARE,    ACTIVE_GRENADE,      ACTIVE_CART,
    ACTIVE_WEAPON, ACTIVE_GENERAL, ACTIVE_COMBAT_SOFT, ACTIVE_DEFENSE_SOFT, ACTIVE_MISC_SOFT,
};
...
Ref smallstuffSpews[] = {NULL_REF, NULL_REF, NULL_REF, REF_STR_gearSpew0,
                         NULL_REF, NULL_REF, NULL_REF, REF_STR_plotSpew0};
...
uchar mfd_general_inv_handler(MFD *m, uiEvent *ev, int row) {
    ...
    if (ev->type != UI_EVENT_MOUSE || !(ev->subtype & MOUSE_LDOWN))
        return FALSE;
...
void mfd_item_expose(MFD *m, ubyte control) {
    ...
    if (currtype == MFD_INV_NOTYPE)
        currclass = MFD_INV_NULL;
    ...
    if (control & MFD_EXPOSE) {
        ...
        if ((control & MFD_EXPOSE_FULL) || (currclass != lastclass) || (currtype != lasttype))
            FullRedraw = TRUE;
...
uchar mfd_item_handler(MFD *m, uiEvent *e) {
    ...
    case MFD_INV_HARDWARE: {
        LGRect r = {{HARDWARE_BUTTON_X, HARDWARE_BUTTON_Y},
                    {HARDWARE_BUTTON_X + HARDWARE_BUTTON_W, HARDWARE_BUTTON_Y + HARDWARE_BUTTON_H}};
        RECT_OFFSETTED_RECT(&r, m->rect.ul, &r);
        if (RECT_TEST_PT(&r, e->pos)) {
...
void mfd_shieldware_expose(MFD *mfd, ubyte control) {
    ...
    if (full)
        draw_mfd_item_spew(REF_STR_wareSpew0 + STRINGS_PER_WARE * n, v);
    if (full || s != SHIELD_LAST_STATUS(mfd->id) || SHIELD_LAST_VERSION(mfd->id) != v) {
...
void mfd_motionware_expose(MFD *mfd, ubyte control) {
    ...
    if (full)
        draw_mfd_item_spew(REF_STR_wareSpew0 + STRINGS_PER_WARE * n, v);
    if (full || s != MOTION_LAST_STATUS(mfd->id) || MOTION_LAST_VERSION(mfd->id) != v) {
...
errtype mfd_grenade_init(MFD_Func *f) {
    ...
    LGRect r = {{GRENADE_SLIDER_X, GRENADE_SLIDER_Y},
                {GRENADE_SLIDER_X + GRENADE_SLIDER_W, GRENADE_SLIDER_Y + GRENADE_SLIDER_H}};
...
errtype mfd_elevator_init(MFD_Func *f) {
    ...
    LGRect r = {{ELEV_BTTNS_X, ELEV_BTTNS_Y}, {ELEV_BTTNS_X + ELEV_BTTNS_WD, ELEV_BTTNS_Y + ELEV_BTTNS_HT}};
...
char *level_to_floor(int lev_num, char *buf) {
    ...
    switch (lev_num) {
    case 0:
        buf[bpos++] = toupper(get_temp_string(REF_STR_ReactorWord)[0]);
        break;
    case 11:
        buf[bpos++] = grov_init;
        buf[bpos++] = '4';
...
errtype mfd_keypad_init(MFD_Func *f) {
    ...
    LGRect r = {{KEYPAD_BTTNS_X, KEYPAD_BTTNS_Y}, {KEYPAD_BTTNS_X + KEYPAD_BTTNS_WD, KEYPAD_BTTNS_Y + KEYPAD_BTTNS_HT}};
    ...
    if ((keypad_data->special > 0) && (keypad_data->curr_digit > 0))
        ...
        if (full || (keypad_data->last_digit != keypad_data->curr_digit)) {
            ...
            for (i = 0; i < NUM_KEYPAD_BUTTONS; i++) {
                ...
                bttn.x = KEYPAD_BTTNS_X +
                         (i / KEYPAD_BTTN_ROWS) * (KEYPAD_BTTNS_WD - KEYPAD_BTTN_WD) / (KEYPAD_BTTN_COLS - 1);
                bttn.y = KEYPAD_BTTNS_Y +
                         (i % KEYPAD_BTTN_ROWS) * (KEYPAD_BTTNS_HT - KEYPAD_BTTN_HT) / (KEYPAD_BTTN_ROWS - 1);

                if ((keypad_data->curr_digit > 0) &&
                    (keypad_num(i) == keypad_data->digits[keypad_data->curr_digit - 1]))
                    clr = SELECTED_ITEM_COLOR;
                else
                    clr = ITEM_COLOR;
                gr_set_fcolor((long)clr);
            ...
            mfd_add_rect(KEYPAD_BTTNS_X, KEYPAD_BTTNS_Y, KEYPAD_BTTNS_X + KEYPAD_BTTNS_WD,
                         KEYPAD_BTTNS_Y + KEYPAD_BTTNS_HT);
            ResUnlock(MFD_FONT);
        }
    }
    POP_CANVAS();
...
void set_inventory_mfd(ubyte obclass, ubyte type, uchar grab) {
    ...
    case MFD_INV_WEAPON:
        ...
        func = MFD_WEAPON_FUNC;
        slot = MFD_WEAPON_SLOT;
        stat = MFD_ACTIVE;
