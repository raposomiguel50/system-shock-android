...
uchar comparator_check(int comparator, ObjID obj, uchar *special_code) {
    ...
    *special_code = 0;
    ...
    case 0:
        truthval = (cval == compval);
        fail_amt = abs(cval - compval);
        break;
    case 1:
        truthval = (cval < compval);
        fail_amt = cval - compval;
        break;
    case 2:
        truthval = (cval <= compval);
        fail_amt = cval - compval;
...
errtype trap_null_func(int p1, int p2, int p3, int p4) { return (OK); }
...
uchar player_facing_square(LGPoint sq) {
    ...
    if (view360_active_contexts[LEFT_CONTEXT] && fixang_abs(delta + 0x4000) < CONSERVATIVE_OCTANT_ARC)
        return (TRUE);
    if (view360_active_contexts[RIGHT_CONTEXT] && fixang_abs(delta - 0x4000) < CONSERVATIVE_OCTANT_ARC)
        return (TRUE);
    if (view360_active_contexts[MID_CONTEXT] && fixang_abs(delta + 0x8000) < CONSERVATIVE_OCTANT_ARC)
        return (TRUE);
    return (FALSE);
}
...
errtype do_ai_trap(ObjSpecID osid, int p1, int p3, int p4) {
    ...
    if ((p1 < 0) || ((objs[oid].subclass == (p1 & 0xFF00) >> 8) && (objs[oid].info.type == (p1 & 0xFF)))) {
        // Now look at the solo bit
        if ((p1 == -1) ||     // we are already OK
            (p1 & 0x20000) || // or we don't care
            ((p1 & 0x10000) &&
             (objs[oid].info.inst_flags & CLASS_INST_FLAG)) || // or if we want only loners, and we're a loner
            ((!(p1 & 0x10000)) &&
             (!(objs[oid].info.inst_flags & CLASS_INST_FLAG)))) // or we want only non-loners, and we're a non-loner
        {
...
errtype trap_ai_func(int p1, int p2, int p3, int p4) {
    ...
    if ((x2 - x1) * (y2 - y1) > SEARCH_AREA_THRESHOLD) {
        ...
        while (osid != OBJ_SPEC_NULL) {
            oid = objCritters[osid].id;
            if ((OBJ_LOC_BIN_X(objs[oid].loc) >= x1) && (OBJ_LOC_BIN_X(objs[oid].loc) <= x2) &&
                (OBJ_LOC_BIN_Y(objs[oid].loc) >= y1) && (OBJ_LOC_BIN_Y(objs[oid].loc) <= y2)) {
...
errtype trap_scheduler_func(int p1, int p2, int p3, int p4) {
    ...
    if ((p3 >= 0xFFFF) || ((p3 > 0x1000) && (QUESTBIT_GET(p3 & 0xFFF))) || ((p3 < 0x1000) && (p3 > 0))) {
        ...
        if ((p3 < 0x1000) && (p3 > 0))
            (*p)--;
        new_event.timestamp =
            TICKS2TSTAMP(player_struct.game_time + (CIT_CYCLE * (ushort)qdata_get(p2)) / TRAP_TIME_UNIT) + 1;
        if (qdata_get(p4) != 0)
...
errtype trap_alternating_splitter_func(int p1, int p2, int p3, int p4) {
    ...
    while (!found_new) {
        switch (n) {
        case 0:
            do_timed_multi_stuff(qdata_get(p1));
            found_new = TRUE;
            break;
        case 1:
            do_timed_multi_stuff(qdata_get(p2));
            found_new = TRUE;
            break;
        case 2:
            do_timed_multi_stuff(qdata_get(p3));
            found_new = TRUE;
            break;
        }
        n += 1;
...
errtype trap_main_light_func(int p1, int p2, int p3, int p4) {
    ...
    if (p2 & 0xFFFF) {
        ...
        switch (objs[current_trap].obclass) {
        case CLASS_TRAP:
            p = &(objTraps[objs[current_trap].specID].p2);
            break;
        case CLASS_FIXTURE:
            p = &(objFixtures[objs[current_trap].specID].p2);
            break;
        }
        *p &= 0xF000FFFF;
...
errtype trap_questbit_func(int p1, int p2, int p3, int p4) {
    ...
    if (p1 & 0x2000) {
        ...
        switch (mod) {
        case 0:
            qdata_set(p1, qdata_get(qarg));
            break;
        case 1:
            qdata_set(p1, qdata_get(p1) + qdata_get(qarg));
            break;
        case 2:
            qdata_set(p1, qdata_get(p1) - qdata_get(qarg));
            break;
...
errtype real_instance_func(int p1, int p2, int p3, int p4) {
    ...
    case CLASS_BIGSTUFF:
        if (p2 != -1)
            objBigstuffs[osid].cosmetic_value = p2;
        if (p3 != -1)
            objBigstuffs[osid].data1 = p3;
        if (p4 != -1)
            objBigstuffs[osid].data2 = p4;
        break;
    case CLASS_DOOR:
        if (p2 != -1)
            objDoors[osid].locked = p2;
        if (p3 != -1) {
...
errtype trap_hack_func(int p1, int p2, int p3, int p4) {
    ...
    case REACTOR_DIGIT_HACK:
        if (objs[p2].obclass == CLASS_BIGSTUFF) {
            ...
            combo = (QUESTVAR_GET(REACTOR_COMBO_QVAR) << 12) | (QUESTVAR_GET(REACTOR_COMBO_QVAR + 1));

            objBigstuffs[ID2SPEC(p2)].data2 = 0x100 | (SCREEN_DIGIT_0 + (0xF & (combo >> (4 * (6 - p3)))));
        }
        break;
...
errtype do_timed_multi_stuff(int p) {
    ...
    if (p >> 16) {
        // Time delay
        new_event.timestamp =
            TICKS2TSTAMP(player_struct.game_time + (ushort)(CIT_CYCLE * (p >> 16)) / MULTI_TIME_UNIT) + 1;
        new_event.type = TRAP_SCHED_EVENT;
...
errtype trap_plot_clock_func(int p1, int p2, int p3, int p4) { return (OK); }
...
errtype (*trap_functions[])(int, int, int, int) = {
    ...
    trap_teleport_func,
    trap_damage_func,
    trap_create_obj_func,
    trap_questbit_func,
    trap_cutscene_func,
    trap_multi_func,
    trap_main_light_func,
    trap_sfx_func,
    trap_height_func,
    trap_terrain_func,
    trap_scheduler_func,
    trap_alternating_splitter_func,
    trap_destroy_object_func,
    trap_plot_clock_func,
    trap_email_func,
    trap_expose_func,
    trap_instance_func,
    trap_animate_func,
    ...
    while (osid != OBJ_SPEC_NULL) {
        if (ID2TRIP(objTraps[osid].id) == LEVEL_TRIG_TRIPLE) {
            if (comparator_check(objTraps[osid].comparator, objTraps[osid].id, &special)) {
                grind_trap(objTraps[osid].trap_type, objTraps[osid].p1, objTraps[osid].p2, objTraps[osid].p3,
                           objTraps[osid].p4, &objTraps[osid].destroy_count, objTraps[osid].id);
            }
        }
        osid = objTraps[osid].next;
    }
