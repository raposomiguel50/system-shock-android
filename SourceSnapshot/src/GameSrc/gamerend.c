...
static uchar systems_line_colors[] = {RED_8_BASE, RED_8_BASE + 3, ORANGE_8_BASE, ORANGE_8_BASE + 3, GREEN_8_BASE};
...
// KLC-duplicate mail               add_email_datamunge(FAKEWIN_EMAIL_MUNGE, FALSE);
    ...
    if (secret_render_fx == 0) // why must we unset both
    {
        ...
        fx_font = NULL;
        chg_unset_sta(GL_CHG_2);
        chg_unset_flg(GL_CHG_2);
    }
}
...
#define DRAW_BEAM_LINE(c1, c2, c3, c4)                                                               \
    {                                                                                                \
        ...
        ss_thick_fix_line(fix_make(a, 0), fix_make(b, 0), fix_make(deltax + (c3) + boff + beamx, 0), \
                          fix_make(deltay + (c4) + boff + 6, 0));                                    \
    }
    ...
    for (line_base = stat_dest->bits, y = 0; y < stat_dest->h; y++, line_base += stat_dest->row) {
        if ((last == LAST_INITIAL) || ((rand() & 0xff) < (dens1 + last))) {
            ...
            if (last == 0)
                ...
                last >>= 1; // decay repeat freq, stop at 1
        } else
            last = 0;
    }
}

void draw_full_static(
    grs_bitmap *stat_dest,
    ...
    if (flags & FR_DOUBLEB_MASK) // looks like a bug to me, eh?
    {
        ...
        case FR_SFX_SHIELD:
            ...
            for (loop = 1; loop < 5; loop++) {
                draw_single_static_line(orig_bits + (orig_h + loop) * dest_bm->w, orig_w + loop,
                                        dest_bm->w - orig_w - loop, BLUE_8_BASE);
                draw_single_static_line(orig_bits + (dest_bm->h - (orig_h + loop)) * dest_bm->w, orig_w + loop,
                                        dest_bm->w - orig_w - loop, BLUE_8_BASE);
            }
