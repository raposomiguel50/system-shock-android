...
void mfd_target_expose(MFD *m, ubyte control) {
    ...
    if (player_struct.curr_target != LAST_TARGET(m->id))
        full = TRUE;
        ...
        if (full && target == OBJ_SPEC_NULL) {
            ...
            if (version >= 2) {
                ...
                if (full || dmg != LAST_STATUS(m->id)) {
                    ...
                    //             siz = mfd_draw_string(get_temp_string(REF_STR_Condition), LEFT_MARGIN, y, TEXT_COLOR,
                    //             TRUE); y += siz.y + 1;
                    siz = mfd_full_draw_string(buf, LEFT_MARGIN, y, TEXT_COLOR, TARGET_FONT, TRUE, TRUE);
                    y += siz.y + 1;
...
uchar iter_eligible_targets(ObjSpecID *sid) {
    ...
    if (*sid == OBJ_SPEC_NULL)
        *sid = objCritters[0].id;
    else
        *sid = objCritters[*sid].next;
    for (; *sid != OBJ_SPEC_NULL; *sid = objCritters[*sid].next) {
        ...
        if (get_crit_posture(*sid) == DEATH_CRITTER_POSTURE || objs[PLAYER_OBJ].specID == *sid)
            continue;
        if (dsq > ELIGIBLE_TARGET_RANGE_SQUARED)
            continue;
        if (ray_cast_objects(PLAYER_OBJ, oid, VISIBLE_MASS, VISIBLE_SIZE, VISIBLE_SPEED,
                             fix_make(ELIGIBLE_TARGET_RANGE, 0)) != oid)
            continue;
        return TRUE;
    }
    return FALSE;
