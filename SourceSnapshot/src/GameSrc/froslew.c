...
int32_t *fr_objslew_obj_to_list(int32_t *flist, Obj *cobj, int count) {
    ...
    case 4:
        flist[4] = (cobj->loc.p << 8);
    case 3:
        flist[3] = (cobj->loc.h << 8);
    case 2:
        flist[2] = (cobj->loc.z << SLOPE_SHIFT_D);
    default:
    case 1:
        flist[1] = cobj->loc.y;
    case 0:
...
Obj *fr_objslew_list_to_obj(int32_t *flist, Obj *cobj, int count) {
    ...
    case 4:
        cobj->loc.p = (flist[4] >> 8);
    case 3:
        cobj->loc.h = (flist[3] >> 8);
    case 2:
        cobj->loc.z = (flist[2] >> SLOPE_SHIFT_D);
    default:
    case 1:
        cobj->loc.y = flist[1];
    case 0:
...
uchar fr_objslew_go_real_height(Obj *cobj, int32_t *eye) {
    ...
    if (cobj != NULL) {
        eye = leye;
        fr_objslew_obj_to_list(eye, cobj, 3);
    }
    x = eye[0] >> MAP_SH;
    y = eye[1] >> MAP_SH;
    o_t = MAP_GET_XY(x, y);
    ...
    if (me_bits_mirror(o_t) != MAP_FFLAT)
        if ((me_tiletype(o_t) >= TILE_SLOPEUP_N) && (me_tiletype(o_t) <= TILE_SLOPEUP_W)) {
            ...
            case TILE_SLOPEUP_E:
                ...
                break;
            case TILE_SLOPEUP_S:
                diff = MAP_MK - (eye[1] & MAP_MK);
                break;
            case TILE_SLOPEUP_W:
                diff = MAP_MK - (eye[0] & MAP_MK);
                break;
            }
...
uchar fr_objslew_moveone(Obj *cobj, ObjID objnum, int which, int how, uchar conform) {
    ...
    case EYE_B:
    case EYE_P:
        eye_mods[which - 3] += how * cam_slew_scale[which];
        return valid_pos; // break;
    case EYE_Z:
        eye[2] += how << SLOPE_SHIFT_D;
        break;
    case EYE_Y:
    case EYE_X: {
        fix v[2], tot;
