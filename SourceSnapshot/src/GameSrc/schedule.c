...
int compare_tstamps(ushort t1, ushort t2) { return expand_tstamp(t1) - expand_tstamp(t2); }
...
static SchedHandler sched_handlers[] = {
    null_event_handler,  grenade_event_handler,  explosion_event_handler, door_event_handler,
    trap_event_handler,  exposure_event_handler, height_event_handler,    height_event_handler,
    light_event_handler, bark_event_handler,     email_event_handler,
};
...
errtype schedule_run(Schedule *s, ushort time) {
    ...
    errtype err;
    current_tstamp = time;
    for (err = pqueue_least(&s->queue, &ev); err == OK && compare_tstamps(ev.timestamp, time) < 0;
