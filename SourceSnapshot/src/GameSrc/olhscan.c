...
void olh_init_single_scan(fauxrend_context **outxt, fauxrend_context *intxt) {
    ...
    *outxt = (fauxrend_context *)fr_place_view(FR_NEWVIEW, FR_DEFCAM, mem,
                                               FR_CURVIEW_STRT | FR_DOUBLEB_MASK | FR_PICKUPM_MASK, 0, 0, 0, 0,
                                               intxt->xwid / SCAN_RATIO, intxt->ywid / SCAN_RATIO);
...
ushort olh_scan_objs(void) {
    ...
    for (y = yl; y < fr->draw_canvas.bm.h; y++)
        for (x = xl; x < fr->draw_canvas.bm.w; x++) {
            if (y > BOTTOM_MARGIN && x > fr->draw_canvas.bm.w - RIGHT_MARGIN)
                break;
            ...
            if ((col >= FR_CUR_OBJ_BASE) && (col < fr_cur_obj_col)) // if we are actually exactly over an object
            {
                ...
                if (olh_candidate(obj)) {
                    ...
                    objdata[i].count = 1;
                found:
                    //               mprintf ("found %d at (%d,%d) (x,y) now (%d,%d), count now
    ...
    if (objcount > 0) {
        ...
        for (i = 0; i < objcount; i++) {
            ...
            uint weight =
                dat->count * (fr->xwid + fr->ywid) / (abs(2 * pos.x - fr->xwid) + abs(2 * pos.y - fr->ywid) + 1);
...
#ifdef SVGA_SUPPORT
        {
            ...
            tmp = fix_mul(tmp, x_mul);
            olh_object.loc.x = fix_int(tmp);
            tmp = fix_make(olh_object.loc.y, 0);
