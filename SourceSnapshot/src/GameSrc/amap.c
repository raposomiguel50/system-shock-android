...
void amap_invalidate(int id) { oAMap(id)->init = FALSE; }
...
void obj_mess(curAMap *amptr, MapElem *curmp, int drw, int xm, int ym, int tsize, int pass) {
    ...
    while (curORef != OBJ_REF_NULL) {
        ...
        if (CitrefCheckHomeSq(curORef)) { // this is the home square... so do stuff
            cobj = &objs[cobjid];
...
void amap_draw(curAMap *amptr, int expose) {
    ...
    if (amptr->sensor_obj != OBJ_NULL) {
        sensor_x = objs[amptr->sensor_obj].loc.x;
        sensor_y = objs[amptr->sensor_obj].loc.y;
    } else
        sensor_x = sensor_y = 0;
    ...
    //   mprintf("rect %d %d and %d %d, crnr %d %d from rw and rh %.2q %.2q cnt %.2q
...
uchar amap_zoom(curAMap *amptr, uchar set, int zoom_delta) {
    if (set)
        ...
        if (zoom_delta > 0) {
            if (amptr->zoom + zoom_delta >= AMAP_MAX_ZOOM)
                return FALSE;
        } else {
            if (amptr->zoom + zoom_delta < AMAP_MIN_ZOOM)
                return FALSE;
        }
        amptr->zoom += zoom_delta;
    }
...
char *amap_str_next(void) { return amap_str_ptr; }

void amap_str_grab(char *str) { amap_str_ptr = str + strlen(str) + 1; }
...
void amap_str_startup(int magic_num) { amap_str_ptr = &amap_strings[0] + magic_num; }
