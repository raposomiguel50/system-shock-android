...
struct _nVecWork _nVP[4] = {{0xdeadbeef, -1, -64, 3, 2, 0xdeadbeef, 0xff, nVW_NXNY},
                            {0xdeadbeef, -1, 64, 3, 0, 0xdeadbeef, 0xff, nVW_NXPY},
                            {0xdeadbeef, 1, -64, 1, 2, 0xdeadbeef, 0xff, nVW_PXNY},
                            {0xdeadbeef, 1, 64, 1, 0, 0xdeadbeef, 0xff, nVW_PXPY}};

// this is a gross way to do this...
#define _fr_build_clip_vec(cv, org, ray, aflags)               \
    (cv)->len = 0;                                             \
    (cv)->flags = ((cv)->flags & FRVECMASK) + aflags;          \
...
static void _fr_rebuild_nVecWork(void) {
    ...
    new_del_mapstep[0] = wall_adds[2];
    new_del_mapstep[1] = wall_adds[0];
}
...
static void _fr_init_vecwork(void) {
    ...
    {
        allclipv[i].flags = i;
        allclipv[i].nxtv = (i + 1) & (MAX_CLIP_VEC - 1);
    }
    vechead = 0;
    ffreevec = 0;
    lastvec = -1;
...
void print_nvp(struct _nVecWork *nvp) {
    ...
    mprintf(" _nVP xrem %x stepy %x. move_x %d std %s %c%c %d %d\n", nvp->remx, nvp->stepy, nvp->move_x,
            nvm_str[nvp->dircode], ifc_str[nvp->inface[0]], ifc_str[nvp->inface[1]], nvp->mapstep[0], nvp->mapstep[1]);
}

void print_fcv(FrClipVec *_ccv, int dim) {
    if (dim == 3)
        mprintf(" _ccv @(%x,%x,%x),d(%x,%x,%x)..", _ccv->loc[0], _ccv->loc[1], _ccv->loc[2], _ccv->deltas[0],
                _ccv->deltas[1], _ccv->deltas[2]);
    else
...
static uchar _sclip_major_mask[9][2] = {{FMK_SW, FMK_EW}, {FMK_SW, FMK_WW}, {FMK_NW, FMK_EW},
                                        {FMK_NW, FMK_WW}, {FMK_NW, FMK_WW}, {FMK_NW, FMK_EW},
                                        {FMK_SW, FMK_WW}, {FMK_SW, FMK_EW}, {0, 0}};

static uchar _sclip_door_mask[9][2] = {{FMK_INT_NW, FMK_INT_EW}, {FMK_INT_NW, FMK_INT_WW}, {FMK_INT_SW, FMK_INT_EW},
                                       {FMK_INT_SW, FMK_INT_WW}, {FMK_INT_SW, FMK_INT_WW}, {FMK_INT_SW, FMK_INT_EW},
                                       {FMK_INT_NW, FMK_INT_WW}, {FMK_INT_NW, FMK_INT_EW}, {0, 0}};
...
uchar _fr_move_ccv_x(struct _nVecWork *nvp) {
    ...
    if (fr_obj_block(ccv->mptr, _sclip_door, (int *)ccv->loc) ||
        ...
        tt = me_tiletype(ccv->mptr);
        if ((_face_nxtedge[tt << 2] == 0xff) || (me_clearsolid(ccv->mptr) & _face_nxtmask) ||
...
void _fr_move_along_dcode(int dircode) {
    ...
    if (dircode & nVW_XDIR)
        nvp->remx = _fixp1 - 1 - fix_frac(ccv->loc[0]);
    else
        nvp->remx = -fix_frac(ccv->loc[0]);

    if (ccv->deltas[1] == 0) // no y delta
    {
        nvp->move_x = TRUE;
        nvp->stepy = 0;
    }                             // lets do the flat line thing
    ...
    {                             // get the signs right, as it were
        if (nvp->remx)
            switch (dircode) { // these just arent right
            case nVW_NXNY:
                nvp->move_x = (-nvp->remx * ccv->deltas[1] > (fix_frac(ccv->loc[1])) * (ccv->deltas[0]));
                break; // flip sign for --
            case nVW_NXPY:
                nvp->move_x = (nvp->remx * ccv->deltas[1] > (_fixp1 - 1 - fix_frac(ccv->loc[1])) * (ccv->deltas[0]));
                break; // flip xd, sign
            case nVW_PXNY:
                nvp->move_x = (-nvp->remx * ccv->deltas[1] < (fix_frac(ccv->loc[1])) * ccv->deltas[0]);
                break; // flip yd, sign
    ...
    if (nvp->move_x) {
        if (dircode & nVW_XDIR) {
            _face_curedge = &face_obstruct[0][1];
            _face_nxtedge = &face_obstruct[0][3];
            _face_curmask = FMK_INT_EW;
            _face_nxtmask = FMK_INT_WW;
        } else {
            _face_curedge = &face_obstruct[0][3];
            _face_nxtedge = &face_obstruct[0][1];
            _face_curmask = FMK_INT_WW;
            _face_nxtmask = FMK_INT_EW;
        }
    ...
    if (move_in_y) { // actually move in y
        fix nstp;
        if (dircode & nVW_YDIR)
            nstp = _fixp1 - 1 - fix_frac(ccv->loc[1]);
        else
            nstp = -fix_frac(ccv->loc[1]);
...
#define is_solid()                                                                                                     \
    ((_face_topedge[(me_tiletype(cv->mptr) << 2)] == 0xff) || (me_clearsolid(cv->mptr) & _face_topmask) ||             \
     (me_subclip(cv->mptr) & _face_topmask) || (_face_botedge[(me_tiletype((cv->mptr + y_map_step)) << 2)] == 0xff) || \
     (me_clearsolid((cv->mptr + y_map_step)) & _face_botmask) ||                                                       \
     (me_subclip(cv->mptr + y_map_step) == SUBCLIP_OUT_OF_CONE))

#define is_space()                                                                                                \
    ((_face_topedge[(me_tiletype(cv->mptr) << 2)] != 0xff) && ((me_clearsolid(cv->mptr) & _face_topmask) == 0) && \
...
#define fixup_delta(dlta)                     \
    if ((dlta) > 0)                           \
        dlta = ((dlta) + DT_FAKE) >> DT_SHFT; \
    else                                      \
        (dlta) = ((dlta)-DT_FAKE) >> DT_SHFT;

void _fr_del_compute(FrClipVec *v1, FrClipVec *v2) {
    v1->deltas[0] = v1->loc[0] - coor(EYE_X);
    v1->deltas[1] = v1->loc[1] - coor(EYE_Y);
    fixup_delta(v1->deltas[0]);
    fixup_delta(v1->deltas[1]);
    v2->deltas[0] = v2->loc[0] - coor(EYE_X);
    v2->deltas[1] = v2->loc[1] - coor(EYE_Y);
...
#endif
            tmpr->loc[0]--;
            tmpr->mptr--; // note that tmpl is a-ok
            if (northward) {
                tmpr->flags |= FRVECUSE | FRVECL;
                tmpl->flags |= FRVECUSE | FRVECR;
            } else {
...
uchar _fr_move_new_dels(FrClipVec *lv, FrClipVec *rv, uchar northward) {
    ...
    rm = rv->nxtv;
    for (; lm != rm; lm = (allclipv + lm)->nxtv)
        (allclipv + lm)->mptr += new_del_mapstep[northward];
    return TRUE;
}
...
void _fr_show_veclist(void) {
    ...
    for (i = 0, cv = vechead; i < MAX_CLIP_VEC; i++, cv = allclipv[cv].nxtv)
        mprintf("%1.1X%c%c ", cv, (allclipv[cv].flags & FRVECUSE) ? 'U' : 'x',
                (allclipv[cv].flags & FRVECL) ? 'L' : 'R');
    if (cv != vechead)
        mprintf("ERROR %d!%d\n", cv, vechead);
    else
        mprintf("\n");
