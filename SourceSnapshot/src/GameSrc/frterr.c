...
void _fr_figure_pt(g3s_phandle tmp, int pt_code) {
    ...
    switch (_fdt_pbase = ptm->base) { // bad bad bad - hmmm... arrays?
    case 0:
        core = _fr_ptbase;
        break;
    case 1:
    ...
    switch (ptm->modcnt) { // 1 function table
    case FRMODYAXIS:
        g3_add_delta_z(tmp, ptm->arg);
        break;
    case FRMODXAXIS:
        g3_add_delta_x(tmp, ptm->arg);
        break;
    case FRMODNONE:
        break;
    }
...
int _fr_do_light(g3s_phandle wrk, int which) {
    ...
    dval =
        fix_fast_pyth_dist(fix_fast_pyth_dist((_lgt_x << 16) - fr_camera_last[0], (_lgt_y << 16) - fr_camera_last[1]),
                           fr_camera_last[2] - _fdt_hgt_val);
    wrk->i = _fr_do_light_val(which, dval);
    return wrk->i;
}

int _obj_do_light(int which, fix dist) { return _fr_do_light_val(which, dist); }

void _fr_do_cspace(g3s_phandle wrk, int which) {
    int col, dst, ncval;
    MapElem *our_mp = _fdt_mptr + csp_trans_add[_fdt_pbase];
    if (which == FRPTSZCEIL_DN)
        col = me_cybcolor_ceil(our_mp);
    else
        col = me_cybcolor_flr(our_mp);
...
static void _fr_tmap_int_wall(int wall_id) {
    ...
    // need to have face_code set and be ready with flip and hold and all that jazz
    _fr_figure_pt(_fdt_tmppts[0], wpt->ul);
    wall_uv_l(_fdt_tmppts[0]);
    _fr_figure_pt(_fdt_tmppts[1], wpt->ur);
    wall_uv_r(_fdt_tmppts[1]);
    _fr_figure_pt(_fdt_tmppts[2], wpt->lr);
    wall_uv_r(_fdt_tmppts[2]);
    _fr_figure_pt(_fdt_tmppts[3], wpt->ll);
...
static void _fr_tmap_lit_int_wall(int wall_id) {
    ...
    // need to have face_code set and be ready with flip and hold and all that jazz
    _fr_figure_pt(_fdt_tmppts[0], wpt->ul);
    wall_uv_l_i(_fdt_tmppts[0]);
    _fr_pt_light(_fdt_tmppts[0]);
    _fr_figure_pt(_fdt_tmppts[1], wpt->ur);
    wall_uv_r_i(_fdt_tmppts[1]);
    _fr_pt_light(_fdt_tmppts[1]);
    _fr_figure_pt(_fdt_tmppts[2], wpt->lr);
    wall_uv_r_i(_fdt_tmppts[2]);
    _fr_pt_light(_fdt_tmppts[2]);
    _fr_figure_pt(_fdt_tmppts[3], wpt->ll);
...
static void _fr_cspace_wire_int_wall(int wall_id) {
    ...
    // need to have face_code set and be ready with flip and hold and all that jazz
    _fr_figure_pt(_fdt_tmppts[0], wpt->ul);
    _fr_pt_cspace(_fdt_tmppts[0]);
    _fr_figure_pt(_fdt_tmppts[1], wpt->ur);
    _fr_pt_cspace(_fdt_tmppts[1]);
    _fr_figure_pt(_fdt_tmppts[2], wpt->lr);
...
static void _fr_tmap_ext_wall(fix pt_list[4][2]) {
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[0], -pt_list[0][1]);
    ext_wall_uv_l(_fdt_tmppts[0], pt_list[0]);
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[3], -pt_list[3][1]);
    ext_wall_uv_l(_fdt_tmppts[3], pt_list[3]);
    _fdt_pbase = _fdt_rbase;
    g3_replace_add_delta_y(*_fdt_rcore, _fdt_tmppts[1], -pt_list[1][1]);
    ext_wall_uv_r(_fdt_tmppts[1], pt_list[1]);
    g3_replace_add_delta_y(*_fdt_rcore, _fdt_tmppts[2], -pt_list[2][1]);
...
static void _fr_tmap_lit_ext_wall(fix pt_list[4][2]) {
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[0], -pt_list[0][1]);
    ext_wall_uv_l_i(_fdt_tmppts[0], pt_list[0]);
    _fdt_hgt_val = pt_list[0][1];
    _fr_do_light(_fdt_tmppts[0], dlC);
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[3], -pt_list[3][1]);
    ext_wall_uv_l_i(_fdt_tmppts[3], pt_list[3]);
    _fdt_hgt_val = pt_list[3][1];
    _fr_do_light(_fdt_tmppts[3], dlF);
    _fdt_pbase = _fdt_rbase;
    g3_replace_add_delta_y(*_fdt_rcore, _fdt_tmppts[1], -pt_list[1][1]);
    ext_wall_uv_r_i(_fdt_tmppts[1], pt_list[1]);
    _fdt_hgt_val = pt_list[1][1];
    _fr_do_light(_fdt_tmppts[1], dlC);
    g3_replace_add_delta_y(*_fdt_rcore, _fdt_tmppts[2], -pt_list[2][1]);
    ext_wall_uv_r_i(_fdt_tmppts[2], pt_list[2]);
    _fdt_hgt_val = pt_list[2][1];
    _fr_do_light(_fdt_tmppts[2], dlF);
    if (quik_draw_tmap_p(4)) {
        _fr_ndbg(NO_REND, _fr_lit_wall_func(4, _fdt_tmppts, (*fr_get_tmap)()));
...
static void _fr_cspace_wire_ext_wall(fix pt_list[4][2]) {
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[0], -pt_list[0][1]);
    _fr_do_cspace(_fdt_tmppts[0], dlC);
    g3_replace_add_delta_y(*_fdt_lcore, _fdt_tmppts[3], -pt_list[3][1]);
    _fr_do_cspace(_fdt_tmppts[3], dlF);
    _fdt_pbase = _fdt_rbase;
    g3_replace_add_delta_y(*_fdt_rcore, _fdt_tmppts[1], -pt_list[1][1]);
...
#define floor_uv(pt, pn)                   \
    {                                      \
        (pt)->uv.u = pt_uv[pn][pt_rot][0]; \
        (pt)->uv.v = pt_uv[pn][pt_rot][1]; \
        (pt)->p3_flags |= PF_U | PF_V;     \
    }
#define ceil_uv(pt, pn)                    \
    {                                      \
        (pt)->uv.u = pt_uv[pn][pt_rot][0]; \
        (pt)->uv.v = pt_uv[pn][pt_rot][1]; \
        (pt)->p3_flags |= PF_U | PF_V;     \
...
#define floor_uv_i(pt, pn)                    \
    {                                         \
        (pt)->uv.u = pt_uv[pn][pt_rot][0];    \
        (pt)->uv.v = pt_uv[pn][pt_rot][1];    \
        (pt)->p3_flags |= PF_U | PF_V | PF_I; \
    }
#define ceil_uv_i(pt, pn)                     \
    {                                         \
        (pt)->uv.u = pt_uv[pn][pt_rot][0];    \
        (pt)->uv.v = pt_uv[pn][pt_rot][1];    \
        (pt)->p3_flags |= PF_U | PF_V | PF_I; \
...
int fr_get_ext_gap(fix *face_l, uchar in_fo, uchar *hgts, uchar *mmptr) {
    ...
    if ((fo_t | fo_b) & FO_HT_MSK) {
        ...
        *(face_l + 1) = *(face_l + 3) = _fr_fhgt_list[hgts[FDT_PT_CEIL]]; // set y's
        if (fo_t & FO_L_PARM)
            *(face_l + 1) -= _fr_fhgt_list[hgts[FDT_PT_PARM]];
        if (fo_t & FO_R_PARM)
            *(face_l + 3) -= _fr_fhgt_list[hgts[FDT_PT_PARM]];
        ...
        *(face_l + 5) = *(face_l + 7) = _fr_fhgt_list[hgts[FDT_PT_FLR]]; // set y's
        if (fo_b & FO_R_PARM)
            *(face_l + 5) += _fr_fhgt_list[hgts[FDT_PT_PARM]];
        if (fo_b & FO_L_PARM)
            *(face_l + 7) += _fr_fhgt_list[hgts[FDT_PT_PARM]];
        if ((*(face_l + 3) == *(face_l + 5)) && (*(face_l + 1) == *(face_l + 7)))
            return 0;
    } else {
...
void _render_3d_walls(int which, int cnt) {
    ...
    case 0:
        _fdt_lcore = _fr_ptbase;
        break;
    case 1:
        _fdt_lcore = _fr_ptnext;
        break;
    case 2:
        _fdt_lcore = _fr_ptnext + 1;
        break;
    case 3:
        _fdt_lcore = _fr_ptbase + 1;
        break;
    }
    switch (_fdt_rbase = pt_deref[(wpt->ur & FRPTSPTOFF)].base) {
    case 0:
        _fdt_rcore = _fr_ptbase;
        break;
    case 1:
        _fdt_rcore = _fr_ptnext;
        break;
    case 2:
        _fdt_rcore = _fr_ptnext + 1;
        break;
    case 3:
        _fdt_rcore = _fr_ptbase + 1;
        break;
    }
    _fdt_lcore += _fdt_x;
    _fdt_rcore += _fdt_x;
    switch (cnt) {
    case 4:
        _fr_terr_ext_wall(final_wall[3]);
    case 3:
        _fr_terr_ext_wall(final_wall[2]);
    case 2:
        _fr_terr_ext_wall(final_wall[1]);
    case 1:
        _fr_terr_ext_wall(final_wall[0]);
    case 0:
        break;
    }
}
...
#define dst_from_i(dwp, iwp)                                           \
    {                                                                  \
        (*(dst_wall + (dwp << 1) + 1)) = (*(i_wall + (iwp << 1) + 1)); \
        (*(dst_wall + (dwp << 1))) = (*(i_wall + (iwp << 1)));         \
    }
#define dst_from_o(dwp, owp)                                           \
    {                                                                  \
        (*(dst_wall + (dwp << 1) + 1)) = (*(o_wall + (owp << 1) + 1)); \
        (*(dst_wall + (dwp << 1))) = (*(o_wall + (owp << 1)));         \
    }
...
static void _fr_parse_wall(int which) {
    ...
    if (are_solid) // totally solid on other side, just do us, baby baby
    {
        ...
        //      if (_fdt_ttf->flags&FRFLRFLG_USEPR)
        //      {
        fcnt = fr_get_ext_gap(&final_wall[0][0][0], _fdt_fo[which], _fdt_hgts,
    ...
    if (ocnt != ANTI_RET_PUNTALL) {
        ...
        if ((ocnt & ANTI_MASK_PUNTBOT) == 0) { // touch up the bottom edge
            if (useocnt != last_ocnt) {        // we want to store the ocnt we actually filled, not ended with
                last_ocnt = useocnt;
                outer_wall[useocnt][1][0] = outer_wall[useocnt][2][0] = fix_make(1, 0);
                outer_wall[useocnt][0][0] = outer_wall[useocnt][2][1] = outer_wall[useocnt][3][0] =
                    outer_wall[useocnt][3][1] = 0;
            }
            useocnt++;
        }
        if ((ocnt & ANTI_MASK_PUNTTOP) == 0)
            use_outer_wall = &outer_wall[0][0][0];
        else {
...
#define wall_check(wmask1, wmask2, w_id)                                                                      \
    if ((_fdt_mask & wmask1) && (_fdt_ttw.wallbits & wmask2) && ((me_clearsolid(_fdt_mptr) & wmask1) == 0) && \
        ((me_subclip(_fdt_mptr) & wmask1) == 0))                                                              \
    _fr_parse_wall(w_id)
#endif
...
void fr_draw_tile(void) {
    ...
    if (_fdt_icnt) {
        ...
        case 1:
            _fr_terr_int_wall(_fdt_ttw.wallbase + 0);
        case 0:
            break;
        }
    }
...
static void (*_fr_tabs_int_wall[FRT_MAX_F])(int wall_id) = {
    _fr_flat_int_wall,        _fr_flat_lit_int_wall,    _fr_tmap_int_wall, _fr_tmap_lit_int_wall,
    _fr_cspace_wire_int_wall, _fr_cspace_full_int_wall, _fr_null_int_wall};
static void (*_fr_tabs_ext_wall[FRT_MAX_F])(fix pt_list[4][2]) = {
    _fr_flat_ext_wall,        _fr_flat_lit_ext_wall,    _fr_tmap_ext_wall, _fr_tmap_lit_ext_wall,
    _fr_cspace_wire_ext_wall, _fr_cspace_full_ext_wall, _fr_null_ext_wall};
static void (*_fr_tabs_flr[FRT_MAX_F])(void) = {_fr_flat_flr,     _fr_flat_lit_flr,    _fr_tmap_flr,
                                                _fr_tmap_lit_flr, _fr_cspace_wire_flr, _fr_cspace_full_flr,
                                                _fr_null_flrceil};
static void (*_fr_tabs_ceil[FRT_MAX_F])(void) = {_fr_flat_ceil,     _fr_flat_lit_ceil,    _fr_tmap_ceil,
                                                 _fr_tmap_lit_ceil, _fr_cspace_wire_ceil, _fr_cspace_full_ceil,
                                                 _fr_null_flrceil};
...
#define AddTmapFlags(fl)                         \
    if (textprops[_game_fr_tmap].friction_climb) \
    fl |= SS_BCD_MISC_CLIMB

uchar wall_pc[] = {SS_BCD_PRIM_NEG_Y | SS_BCD_TYPE_WALL, SS_BCD_PRIM_NEG_X | SS_BCD_TYPE_WALL,
...
static void _render_tfunc_walls(int which, int cnt) {
    ...
    {              // NESW
    case 0:
        pt[2] = fix_1 - tf_loc_pt[1];
        pt[0] = tf_loc_pt[0];
        break;
    case 1:
        pt[2] = fix_1 - tf_loc_pt[0];
        pt[0] = fix_1 - tf_loc_pt[1];
        break;
    case 2:
        pt[2] = tf_loc_pt[1];
        pt[0] = fix_1 - tf_loc_pt[0];
        break;
    case 3:
        pt[2] = tf_loc_pt[0];
        pt[0] = tf_loc_pt[1];
    ...
    switch (cnt) {
    case 4:
        tf_solve_aligned_face(pt, final_wall[3], pc, NULL);
    case 3:
        tf_solve_aligned_face(pt, final_wall[2], pc, NULL);
    case 2:
        tf_solve_aligned_face(pt, final_wall[1], pc, NULL);
    case 1:
        tf_solve_aligned_face(pt, final_wall[0], pc, NULL);
    case 0:
        break;
    }
}
...
static void _fr_tfunc_diag_wall(int wall_id) {
    ...
    case 0: // nw - se, normal to upper ne
        diag_norms[0] = fix_inv_root2;
        diag_norms[1] = fix_inv_root2;
        C = tf_loc_pt[0] - tf_loc_pt[1];
        pt[0] = (fix_1 - C) / 2;
        pt[2] = tf_loc_pt[1] - pt[0];
        break;
...
int _fr_tfunc_flr_normal(fix *vec, int *lflg, int fnorm_entry) {
    ...
    case FRFNORM_VZERO:
    case FRFNORM_VFULL:
        *lflg |= SS_BCD_PRIM_ZAXIS | SS_BCD_TYPE_FLOOR;
        return 0;

    case FRFNORM_SLPS:
        vec[0] = 0;
        vec[1] = slope_norm[_fdt_hgts[FDT_PT_PARM]][2];
        rv = 1;
        break;
    case FRFNORM_SLPW:
        vec[0] = slope_norm[_fdt_hgts[FDT_PT_PARM]][2];
        vec[1] = 0;
        rv = 2;
        break;
    case FRFNORM_SLPN:
        vec[0] = 0;
        vec[1] = slope_norm[_fdt_hgts[FDT_PT_PARM]][0];
        rv = 3;
        break;
    case FRFNORM_SLPE:
        vec[0] = slope_norm[_fdt_hgts[FDT_PT_PARM]][0];
        vec[1] = 0;
        rv = 4;
        break;
    }
...
int _fr_tfunc_ceil_normal(fix *vec, int *lflg, int fnorm_entry) {
    ...
    case MAP_CFLAT:
        ...
        break;
    case MAP_MIRROR:
        fnorm_entry = fnorm_entry ^ 0x2; // fall through to the mask
    case MAP_FFLAT:
    case MAP_MATCH:
        fnorm_entry &= 0xf;
    }
    ...
    case FRFNORM_VF_MIR:
    case FRFNORM_VFULL:
        *lflg |= SS_BCD_PRIM_NEG_Z | SS_BCD_TYPE_CEIL;
        return 0;
...
void do_floor_element(int nrm_mask) {
    ...
    case 0:
        ...
        *(g3s_vector *)_tfunc_rpts =
            *(g3s_vector *)tf_loc_pt; // sub faster version?      _memcpy32l(_tfunc_rpts,tf_loc_pt,3);
        _tfunc_rpts[2] = xy[1];
        _tfunc_real_floor[0][1] = _tfunc_real_floor[1][1] = fix_1;
        tf_solve_aligned_face(_tfunc_rpts, _tfunc_real_floor, _tfunc_flg | TF_FLG_BOX_FULL, NULL);
        return; // flat floor
    case 1:
        xy[0] = tf_loc_pt[1];
        _tfunc_rpts[0] = tf_loc_pt[0];
        sc[1] = _tfunc_nrm[1];
        break; // N up (slpS) slope
    case 2:
        xy[0] = tf_loc_pt[0];
        _tfunc_rpts[0] = fix_1 - tf_loc_pt[1];
        sc[1] = _tfunc_nrm[0];
        break; // E up slope
    case 3:
        xy[0] = fix_1 - tf_loc_pt[1];
        _tfunc_rpts[0] = fix_1 - tf_loc_pt[0];
        sc[1] = -_tfunc_nrm[1];
        break; // S up slope
    case 4:
        xy[0] = fix_1 - tf_loc_pt[0];
        _tfunc_rpts[0] = tf_loc_pt[1];
        sc[1] = -_tfunc_nrm[0];
        break; // W up slope
    }
    sc[0] = _tfunc_nrm[2];
...
void do_ceil_element(int nrm_mask) {
    ...
    case 0:
        _tfunc_rpts[0] = tf_loc_pt[0];
        _tfunc_rpts[1] = fix_1 - tf_loc_pt[1];
        _tfunc_rpts[2] = xy[1];
        _tfunc_real_floor[0][1] = _tfunc_real_floor[1][1] = fix_1;
        tf_solve_aligned_face(_tfunc_rpts, _tfunc_real_floor, _tfunc_flg | TF_FLG_BOX_FULL, NULL);
        return; // flat ceil
...
static void _fr_tfunc_ceil(void) {
    ...
    if (fcecnt == 0)
        ...
        if (tmp == MAP_CFLAT)
            hard = 0;
        else if (tmp == MAP_MIRROR)
            hard = !icky;
        else
            hard = icky;
        if (hard) { // broken, depends on icky
...
static void _fr_tfunc_obj(void) { facelet_parse_obj(); }
...
int get_edge_code(void *omp, int edge) {
    ...
    if (!edge_get_fandc(mp, edge, &edge_list[0])) {
        edge_vals[0] = edge_vals[1] = edge_vals[2] = 0;
