...
pt_mods pt_deref[FRPTSUNIQUE] = {
    {0, 0, 0},
    {0, 1, fix_make(0, 0x2000)},
    {0, 1, fix_make(0, 0x8000)},
    {0, 1, fix_make(0, 0xE000)},
    {1, 0, 0},
    {3, 1, fix_make(0, 0x2000)},
    {3, 1, fix_make(0, 0x8000)},
    {3, 1, fix_make(0, 0xE000)},
    {2, 0, 0},
    {0, 2, fix_make(0, 0x2000)},
    {0, 2, fix_make(0, 0x8000)},
    {0, 2, fix_make(0, 0xE000)},
    {3, 0, 0},
    {1, 2, fix_make(0, 0x2000)},
    {1, 2, fix_make(0, 0x8000)},
    {1, 2, fix_make(0, 0xE000)},
    {0, 1, fix_make(0, FROCTNUM)},
    {0, 1, fix_make(0, 0x10000 - FROCTNUM)},
    {3, 1, fix_make(0, FROCTNUM)},
    {3, 1, fix_make(0, 0x10000 - FROCTNUM)},
    {0, 2, fix_make(0, FROCTNUM)},
    {0, 2, fix_make(0, 0x10000 - FROCTNUM)},
    {1, 2, fix_make(0, FROCTNUM)},
    {1, 2, fix_make(0, 0x10000 - FROCTNUM)}
};
...
#define wall_pt(z1, z2, z3, z4, h1, h2, h3, h4) \
    { z1 | h1, z2 | h2, z3 | h3, z4 | h4 }
#define norm_wall(lft, rgt) wall_pt(zTo, zTo, zBo, zBo, lft, rgt, rgt, lft)
#define oct_wall(tl, tr, br, bl)                                                              \
    wall_pt(zTo, zTo, zTD, zTD, tl, tr, br, bl), wall_pt(zTD, zTD, zBU, zBU, bl, br, br, bl), \
        wall_pt(zBU, zBU, zBo, zBo, bl, br, tr, tl)
#define parm_walls(lft, rgt) \
    wall_pt(zBU, zBU, zBo, zBo, lft, rgt, rgt, lft), wall_pt(zTo, zTo, zTD, zTD, lft, rgt, rgt, lft)
...
TilesToWalls tile_walls[FRTILEWALLCNT] = {
    ...
    // basic slopes
    tTw(wN | wE | wS | wW, 0, 0), tTw(wN | wE | wS | wW, 0, 0),
    tTw(wN | wE | wS | wW, 0, 0), tTw(wN | wE | wS | wW, 0, 0),
    // zany slopes
    tTw(wN | wE | wS | wW, 0, 0), tTw(wN | wE | wS | wW, 0, 0),
    tTw(wN | wE | wS | wW, 0, 0), tTw(wN | wE | wS | wW, 0, 0),
    ...
    tTw(wN | wE | wS | wW, 0, 0), tTw(wN | wE | wS | wW, 0, 0),
    // diagonal splits
    tTw(wN | wE | wS | wW, 2, 46), tTw(wN | wE | wS | wW, 2, 50),
    tTw(wN | wE | wS | wW, 2, 48), tTw(wN | wE | wS | wW, 2, 44),
    // oct
...
#define fF3(p1, p2, p3) \
    { 0, 3, 0x##p1, 0x##p2, 0x##p3, 0, 0, 0 }
#define fF4(p1, p2, p3, p4) \
    { 0, 4, 0x##p1, 0x##p2, 0x##p3, 0x##p4, 0, 0 }
#define fT4(p1, p2, p3, p4) \
    { flgNT, 4, 0x##p1, 0x##p2, 0x##p3, 0x##p4, 0, 0 }

#define fS3cc(pl, p2, p3, p4, p5) \
    { flg2 | flgP, 3, zPa | (0x##p2), zPa | (0x##p3), 0x##pl, zPa | (0x##p4), zPa | (0x##p5), 0x##pl }
#define fS3cv(ph, p2, p3, p4, p5) \
    { flg2 | flgP, 3, 0x##p2, 0x##p3, zPa | (0x##ph), 0x##p4, 0x##p5, zPa | (0x##ph) }
#define fS4(p1, p2, p3, p4) \
    { flgP, 4, zPa | (0x##p1), zPa | (0x##p2), 0x##p3, 0x##p4, 0, 0 }
#define fSspl(pla, pha, pm1, pm2) \
    { flg2 | flgP, 3, 0x##pla, 0x##pm1, 0x##pm2, zPa | (0x##pha), zPa | (0x##pm2), zPa | (0x##pm1) }
...
int fr_tables_build(void) {
    ...
    for (i = 0; i < FRFACEOBSTRUCTCNT; i++)
        for (j = 0; j < FACE_CNT; j++) {
            if ((j == 1) || (j == 2))
                face_baseobstruct[i][j] =
                    (face_obstruct[i][j] ^ 0x3F) - 0x9; /* xors to get 7complement, then sub 0b01001 to get 6comp */
            else
