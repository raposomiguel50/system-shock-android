...
void hud_report_damage(ObjID target, byte dmglvl) {
    ...
    if ((dmglvl != DAMAGE_NONE) && (dmglvl != DAMAGE_INEFFECTIVE) // ineffective flag - minman
        && (dmglvl != DAMAGE_STUN) && (dmglvl != DAMAGE_TRANQ) && (ver < TARG_EFF_VERSION)) {
        if (ver == 0)
            return;
        dmglvl = -1;
    }
    for (i = 0; i < NUM_HUD_CRITTERS; i++) {
        if (hud_critters[i].id == target || hud_critters[i].id == OBJ_NULL) {
            best = i;
            break;
        }
...
void hud_do_objs(short xtop, short ytop, short xwid, short ywid, uchar reverse) {
    ...
    for (i = 0; i < current_num_hudobjs; i++) {
        ...
        if (dat->id == player_struct.curr_target) {
            ...
            draw_target_box(dat->xl - targ_frame * w, dat->yl - targ_frame * h, dat->xh + targ_frame * w,
                            dat->yh + targ_frame * h);
            if (targ_frame > 0)
                targ_frame--;
            dat->id = OBJ_NULL;
