...
uchar find_winning_move_from(gridFlowPuzzle *gfpz, gridFlowPuzzle *solved, LGPoint *move, int r0, int c0, int dep,
                             ulong timeout);
uchar find_winning_move(gridFlowPuzzle *gfpz, gridFlowPuzzle *solved, int depth, LGPoint *move, uchar breadth_first,
                        ulong timeout);
...
int gpz_base_colors[] = {BLUE_BASE + 10, GREEN_BASE + 7, 0x24, GREEN_YELLOW_BASE + 9, GRAY_BASE + 5};
...
void wirepos_setup_buttons(wirePosPuzzle *wppz) {
    ...
    LGRect r = {{ACCESSP_BTN_X, ACCESSP_BTN_Y}, {ACCESSP_BTN_X + ACCESSP_FULL_WD, ACCESSP_BTN_Y + ACCESSP_FULL_HGT}};

    bdims.y = wppz->pincnt;
    r.lr.y = ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * wppz->pincnt);
    MFDBttnArrayResize(&(mfd_funcs[MFD_ACCESSPANEL_FUNC].handlers[0]), &r, bdims, bsize);
}

// a1 is master data, a2 target pos, a3 current pos
uchar wirepos_3int_init(wirePosPuzzle *wppz, int a1, int a2, int a3) { // restore the puzzle, for now just init it
    int i;
    wppz->wirecnt = a1 & 0xf;
    a1 >>= 4;
    if (wppz->wirecnt == 0)
    ...
    if (wppz->tscore == 0)
        wppz->tscore = 128;
    wppz->scorealg = a1 & 0xf;
    a1 >>= 4;
    wppz->have_won = a1 & 0xf;
    a1 >>= 4;
    ...
    for (i = 0; i < wppz->wirecnt; i++) {
        wppz->wires[i].cur.lpos = a3 & 0x7;
        wppz->left_tap |= (1 << (a3 & 7));
        a3 >>= 3;
        wppz->wires[i].cur.rpos = a3 & 0x7;
        wppz->right_tap |= (1 << (a3 & 7));
        a3 >>= 3;
        wppz->wires[i].targ.lpos = a2 & 0x7;
        a2 >>= 3;
        wppz->wires[i].targ.rpos = a2 & 0x7;
        a2 >>= 3;
    }
    wirepos_score(wppz);
    wppz->wires_moved = 1;
            ...
            )
   {
                ...
                if (loc == -1) {
                    ...
                    if (((1 << twire) & wim_tap) != 0) {
                        ...
                        if (wim_code & LR_MASK) {
                            wppz->wires[loc].cur.rpos = twire;
                            wppz->wires[oloc].cur.rpos = owire;
                        } else {
                            wppz->wires[loc].cur.lpos = twire;
...
#endif
                    {
                        ...
                        wim_tap |= (1 << twire);
                        if (wim_code & LR_MASK) {
                            wppz->right_tap = wim_tap;
                            wppz->wires[loc].cur.rpos = twire;
                        } else {
                            wppz->left_tap = wim_tap;
                            wppz->wires[loc].cur.lpos = twire;
                        }
                    }
                }
...
int wirepos_rescore_n_check(wirePosPuzzle *wppz) { // score and check, if solved, open us
...
uchar mfd_accesspanel_button_handler(MFD *mfd, LGPoint bttn, uiEvent *ev, void *data) {
    ...
    if (wppz->wire_in_motion != NO_WIRE_IN_MOTION) { // put it down
...
#define accessp_score_rect_f(f, ls, rs, off)                                       \
    f(ACCESSP_SCORE_X - off + ((ls) >> ACCESSP_SCORE_SHF), ACCESSP_SCORE_YT - off, \
      ACCESSP_SCORE_X + off + ((rs) >> ACCESSP_SCORE_SHF), ACCESSP_SCORE_YB + off)

#define accessp_score_rect(ls, rs, off)                                          \
    ACCESSP_SCORE_X - off + ((ls) >> ACCESSP_SCORE_SHF), ACCESSP_SCORE_YT - off, \
        ACCESSP_SCORE_X + off + ((rs) >> ACCESSP_SCORE_SHF), ACCESSP_SCORE_YB + off

#define accessp_score_cmp(sc1, sc2, cmp) ((sc1 >> ACCESSP_SCORE_SHF) cmp(sc2 >> ACCESSP_SCORE_SHF))
...
void mfd_accesspanel_expose(MFD *mfd, ubyte control) {
    ...
    if (control & MFD_EXPOSE) // Time to draw stuff
    {
        ...
        if (full) {
        ...
        } else if (accessp_score_cmp(wppz->last_score, cscore, <)) { // draw more score delta
            gr_set_fcolor(ACCESSP_SCORE_COL);
            ...
            if (accessp_score_cmp(wppz->tscore, wppz->last_score, >=) && accessp_score_cmp(cscore, wppz->tscore, >=)) {
                gr_set_fcolor(ACCESSP_TARG_COL);
                ss_vline(ACCESSP_SCORE_X + (wppz->tscore >> ACCESSP_SCORE_SHF) - 1, ACCESSP_SCORE_YT,
                         ACCESSP_SCORE_YB - 1);
            }
        } else if (accessp_score_cmp(wppz->last_score, cscore, >)) { // put back correct part of background
        ...
        if (full || wppz->wires_moved) { // draw wires
            int i;
            ...
            for (i = 0; i < wppz->pincnt; i++) {
                ...
                ss_box(ACCESSP_BTN_X + 1, ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * i), ACCESSP_BTN_X + ACCESSP_BTN_WD,
                       ACCESSP_BTN_Y - 1 + (ACCESSP_BTN_HGT * (i + 1)));
                ...
                ss_box(ACCESSP_BTN_X, ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * i) - 1, ACCESSP_BTN_X + ACCESSP_BTN_WD + 1,
                       ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * (i + 1)));
            ...
            ss_box(ACCESSP_BTN_X + ACCESSP_BTN_WD, ACCESSP_BTN_Y, ACCESSP_BTN_X + ACCESSP_FULL_WD - ACCESSP_BTN_WD,
                   lry - 1);
            ...
            for (i = 0; i < wppz->wirecnt; i++) {
                ...
                ss_thick_int_line(ACCESSP_BTN_X + (ACCESSP_BTN_WD / 2),
                                  ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * lfti) + (ACCESSP_BTN_HGT / 2) - 1,
                                  ACCESSP_BTN_X + ACCESSP_FULL_WD - 1 - (ACCESSP_BTN_WD / 2),
                                  ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * rghi) + (ACCESSP_BTN_HGT / 2) - 1);
                gr_set_fcolor(basecol + 3);
                ss_thick_int_line(ACCESSP_BTN_X + (ACCESSP_BTN_WD / 2),
                                  ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * lfti) + (ACCESSP_BTN_HGT / 2),
                                  ACCESSP_BTN_X + ACCESSP_FULL_WD - 1 - (ACCESSP_BTN_WD / 2),
                                  ACCESSP_BTN_Y + (ACCESSP_BTN_HGT * rghi) + (ACCESSP_BTN_HGT / 2));
            }
        }
        if ((wppz->wire_in_motion != NO_WIRE_IN_MOTION) &&
            (((*tmd_ticks) & ACCESSP_BLINK_MASK) !=
             wppz->wim_tick)) { // if already shown but not in motion, it is already redrawn, baby
            ...
            if ((wppz->wim_tick == ACCESSP_BLINK_MASK) || (wppz->wire_in_motion == NO_WIRE_IN_MOTION)) {
                ...
                gr_set_fcolor(ACCESSP_PIN_COL - 2);
                wppz->wim_shown = 1;
            }
            if (wppz->wire_in_motion & LR_MASK) {
                ...
                mfd_add_rect(ACCESSP_BTN_X + ACCESSP_FULL_WD - ACCESSP_BTN_WD + 1,
                             ACCESSP_BTN_Y + 1 + (ACCESSP_BTN_HGT * i), ACCESSP_BTN_X + ACCESSP_FULL_WD - 1 - 1,
                             ACCESSP_BTN_Y - 1 - 1 + (ACCESSP_BTN_HGT * (i + 1)));
            } else {
...
uchar gpz_is_charged(gpz_state s) { return (s != GPZ_OPEN && (s & 1)); }
...
errtype mfd_gridpanel_init(MFD_Func *f) {
    ...
    LGRect r = {{GRIDP_BTN_X, GRIDP_BTN_Y}, {GRIDP_BTN_X + GRIDP_FULL_WD, GRIDP_BTN_Y + GRIDP_FULL_HGT}};
...
uchar mfd_solve_gridpanel() {
    ...
    shadow = (WE_ARE_SIGNED() && SHADOW_PANEL_ID == id && SHADOW_PANEL_LEV == player_struct.level &&
              !(objFixtures[objs[id].specID].p2 & 0x10000));
    if (shadow) {
...
gpz_state gpz_get_grid_state(gridFlowPuzzle *gfpz, short row, short col) {
    ...
    if (row < 0 || col < 0 || row >= (gfpz->gfLayout.rows) || col >= (gfpz->gfLayout.cols)) {
        ...
        if (gfpz->gfLayout.src & GPZ_PERIM_TYPEMASK)
            q = &c;
        else
            q = &r;
            ...
            for (c = c0; c < gfpz->gfLayout.cols && dep > 0; c++) {
                ...
                if (s == pass) {
                    ...
                    if (gfpz->gfLayout.control_alg == GPZ_SIMPLE) {
                        ...
                        if (new_r0 < gfpz->gfLayout.rows &&
                            find_winning_move_from(&copy, solved, move, new_r0, new_c0, dep - 1, timeout)) {
                            return TRUE;
                        } else {
...
void gpz_setup_buttons(gridFlowPuzzle *gfpz) {
    LGPoint bsize = {GRIDP_BTN_WD, GRIDP_BTN_HGT};
    LGPoint bdims = {GRIDP_BTN_COL, GRIDP_BTN_ROW};
    LGRect rct = {{GRIDP_BTN_X, GRIDP_BTN_Y}, {GRIDP_BTN_X + GRIDP_FULL_WD, GRIDP_BTN_Y + GRIDP_FULL_HGT}};
    ...
    MFDBttnArrayResize(&(mfd_funcs[MFD_GRIDPANEL_FUNC].handlers[0]), &rct, bdims, bsize);
}
...
void gpz_4int_init(gridFlowPuzzle *gfpz, uint p1, uint p2, uint p3, uint p4) {
    ...
    gfpz->gfLayout.have_won = p3 & 0xF;
    p3 = p3 >> 4;
    gfpz->gfLayout.src = p3 & 0xFF;
    p3 = p3 >> 8;
    gfpz->gfLayout.dest = p3 & 0xFF;
    p3 = p3 >> 8;
    gfpz->gfLayout.cols = cc = p3 & 0xF;
    p3 = p3 >> 4;
    gfpz->gfLayout.rows = rr = p3 & 0xF;
    p3 = p3 >> 4;
...
gpz_state gridpanel_move(LGPoint node, gridFlowPuzzle *gfpz) {
    ...
    if (s == GPZ_OPEN || s == GPZ_CLOSED) {
        ...
        case GPZ_ROOK:
            ...
            if (gfpz->gfLayout.control_alg == GPZ_ROOK)
                break; // queen falls through to bishop.
        case GPZ_BISH:
            sum = node.x + node.y;
            dif = node.x - node.y;
            for (r = 0; r < gfpz->gfLayout.rows; r++) {
...
void mfd_gridpanel_set_winmove(uchar check) {
    ...
    if (check && player_struct.mfd_all_slots[MFD_INFO_SLOT] != MFD_GRIDPANEL_FUNC)
        return;
                    ...
                    for (dr = -1; dr <= 1; dr++) {
                        for (dc = -1; dc <= 1; dc++) {
                            if (dr != 0 && dc != 0)
                                continue;
                            nearstate = gpz_get_grid_state(gfpz, r + dr, c + dc);
                            if (gpz_is_charged(nearstate) || (gpz_is_charged(s) && nearstate == GPZ_GATE)) {
