...
errtype physics_set_player_controls(int bank, byte xvel, byte yvel, byte zvel, byte xyrot, byte yzrot, byte xzrot) {
    if (xvel != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_XVEL] = xvel;
    if (yvel != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_YVEL] = yvel;
    if (zvel != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_ZVEL] = zvel;
    if (xyrot != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_XYROT] = xyrot;
    if (yzrot != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_YZROT] = yzrot;
    if (xzrot != CONTROL_NO_CHANGE)
        player_controls[bank][CONTROL_XZROT] = xzrot;
    return OK;
}
...
void physics_set_relax(int axis, uchar relax) { control_relax[axis] = relax; }
...
#define ITER_OBJSPECS(pmo, objspec) \
...
errtype run_cspace_collisions(ObjID obj, ObjID exclude, ObjID exclude2) {
    ...
    while (oref != OBJ_REF_NULL) {
        ...
        if ((oid != obj) && (oid != exclude) && (oid != exclude2) && (objs[oid].obclass != CLASS_PHYSICS)) {
            ...
            if ((abs(l1.x - l2.x) < use_dist) && (abs(l1.y - l2.y) < use_dist) &&
...
errtype physics_run(void) {
    ...
    if (physics_running && time_passes) {
        ...
        if (motionware_mode == MOTION_SKATES && damp > 1)
            damp--;
        ...
        if (player_struct.foot_planted) {
            ...
            if (ycntl <= SPRINT_CONTROL_THRESHOLD)
                ...
                plr_y = (ycntl - SPRINT_CONTROL_THRESHOLD) * (MAX_SPRINT - MAX_JOG) /
                            (CONTROL_MAX_VAL - SPRINT_CONTROL_THRESHOLD) +
                        MAX_JOG;
            plr_y /= damp;
        ...
        if (time_diff > fix_make(CIT_CYCLE, 0) / MIN_FRAME_RATE)
            time_diff = fix_make(CIT_CYCLE, 0) / MIN_FRAME_RATE;
            ...
            if ((oid = physics_handle_to_id(i)) != OBJ_NULL) // valid ph, get oid
            {
                ...
                if ((oid == PLAYER_OBJ) && global_fullmap->cyber) {
                    ...
                    if (((new_deltah < CYB_VEL_DELTA) && (new_deltap < CYB_VEL_DELTA)) || new_cyber_orient ||
                        ((abs(last_deltah - new_deltah) < CYB_VEL_DELTA2) &&
                         (abs(last_deltap - new_deltap) < CYB_VEL_DELTA2))) {
                        old_head = new_head;
                        old_pitch = new_pitch;
                ...
                if (oid == PLAYER_OBJ) {
                    if (hack_takeover &&
                        (some_move || (abs(newloc.x - objs[oid].loc.x) + abs(newloc.y - objs[oid].loc.y) +
                                           abs(newloc.z - objs[oid].loc.z) >
...
#endif
                {
                    if (me_tiletype(MAP_GET_XY(OBJ_LOC_BIN_X(newloc), OBJ_LOC_BIN_Y(newloc))) == TILE_SOLID) {
                        ...
                        allow_move = FALSE;
                    } else if (new_state.Z < fix_from_map_height(me_height_flr(
                                                 MAP_GET_XY(OBJ_LOC_BIN_X(newloc), OBJ_LOC_BIN_Y(newloc))))) {
...
fix ID2radius(ObjID id) {
    ...
    if (rad > 0)
        return fix_make(rad, 0) / PHYSICS_RADIUS_UNIT;
    else
        return standard_robot.size;
}
...
uchar player_throw_object(ObjID proj_id, int x, int y, int lastx, int lasty, fix vel) {
    ...
    ray_cast_vector(PLAYER_OBJ, &locvec, posvec, THROW_RAYCAST_MASS, radius, THROW_RAYCAST_SPEED,
                    THROW_DISPLACE_RANGE + radius);
...
uchar get_phys_info(int ph, fix *list, int cnt) {
    ...
    default:
        if (cnt <= 0)
            return FALSE;
    case 5:
        list[5] = fixrad_to_fixang(new_state.gamma);
    case 4:
        list[4] = fixrad_to_fixang(-new_state.beta);
    case 3:
        list[3] = fixang_from_phys_angle(new_state.alpha);
    case 2:
        list[2] = new_state.Z;
    case 1:
        list[1] = new_state.Y;
...
errtype collide_objects(ObjID collision, ObjID victim, int bad) {
    ...
    if (objs[collision].obclass == CLASS_GRENADE) {
        ...
        if (owner != PLAYER_OBJ) {
            ...
            case CLASS_CRITTER:
                attack_object(victim, CritterProps[cp_num].attacks[a].damage_type,
                              CritterProps[cp_num].attacks[a].damage_modifier,
                              CritterProps[cp_num].attacks[a].offense_value,
                              CritterProps[cp_num].attacks[a].penetration, 0, 100, NULL, 0, 0, NULL);
                break;
            default:
                if (global_fullmap->cyber && ICE_ICE_BABY(owner)) {
                    attack_object(victim, CYBER_PROJECTILE_TYPE, ice_damage_modifiers[a], ice_offense_values[a],
                                  ice_penetration[a], 0, 100, NULL, 0, 0, NULL);
                }
                break;
            }
        } else {
...
void cit_awol_callback(physics_handle caller) {
    ...
    if ((fix_int(s.X) > global_fullmap->x_size) || (s.X < 0) || (fix_int(s.Y) > global_fullmap->y_size) || (s.Y < 0)) {
        if (caller != -1) {
...
void instantiate_dirac(int triple, Dirac_frame *new_dirac) {
    ...
    *new_dirac = standard_dirac;
    ...
    case LEVEL_GRAV_LOW:
        ...
        break;
    case LEVEL_GRAV_ZERO:
        WARN("%s: Zero gravity level!", __FUNCTION__);
        new_dirac->gravity = 0;
        break;
    default:
        //         if (global_fullmap->cyber)
