...
int random_bell_modifier(uchar attack_on_player) {
    ...
    ubyte difficulty = (global_fullmap->cyber) ? player_struct.difficulty[CYBER_DIFF_INDEX]
                                               : player_struct.difficulty[COMBAT_DIFF_INDEX];
...
ubyte damage_player(int damage, ubyte dtype, ubyte flags) {
    ...
    cur_hp = (global_fullmap->cyber) ? &player_struct.cspace_hp : &player_struct.hit_points;
    ...
    if ((*cur_hp) <= damage) {
        ...
        {
            if (global_fullmap
                    ->cyber) { // No matter what, the player can't be killed in one shot.....it takes at least two shots
...
int compute_damage(ObjID target, int damage_type, int damage_mod, ubyte offense, ubyte penet, int power_level,
    ...
    if (affect) {
        ...
        if ((effect_row != NULL) && (attack_effect_type != SPECIAL_TYPE) && (effect != NULL) &&
...
int get_damage_estimate(ObjSpecID osid) {
    ...
    return (DAMAGE_MAX - ((objs[id].info.current_hp * DAMAGE_MAX) / ObjProps[OPTRIP(triple)].hit_points));
}
...
ubyte attack_object(ObjID target, int damage_type, int damage_mod, ubyte offense, ubyte penet, ubyte flags,
                    int power_level, ubyte *effect_row, ubyte *effect, ubyte attack_effect_type,
    ...
    diff = (global_fullmap->cyber) ? player_struct.difficulty[CYBER_DIFF_INDEX]
                                   : player_struct.difficulty[COMBAT_DIFF_INDEX];
...
ubyte player_attack_object(ObjID target, int wpn_triple, int power_level, Combat_Pt origin) {
    ...
    if ((objs[target].obclass == CLASS_CRITTER) && (get_player_ware_version(WARE_HARD, HARDWARE_TARGET) > 3) &&
        (player_struct.curr_target == OBJ_NULL) && (!global_fullmap->cyber)) {
    ...
    dead = attack_object(target, dtype, damage_mod, offense, penet, flags, power_level, effect_row, &effect,
                         attack_effect_type, &damage_inflicted);
        ...
        //#endif
        {
            ...
            if (ObjProps[OPNUM(target)].render_type == 1) {
                ...
                if (ObjProps[OPNUM(target)].physics_xr > 20) {
                    ...
                    loc.z +=
                        ((RndRange(&damage_rnd, 0, 0x10) - 0x08) +
