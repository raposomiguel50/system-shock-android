...
uchar door_locked(ObjID obj) {
    ...
    if (!DOOR_CLOSED(obj))
        return FALSE;
    if (objDoors[spec].access_level != COMPAR_ESC_ACCESS && objDoors[spec].access_level != 0) {
        int i;
...
uchar use_door(ObjID id, uchar in_inv, ObjID cursor_obj) {
...
#endif
    {
        ...
        try_combo = 1 << objDoors[objs[id].specID].access_level;
        {
            ...
            if (!use_card) {
                if (!(in_inv & 0x2))
                    obj_access_fail_message(REF_STR_DoorWrongAccess, objDoors[objs[id].specID].access_level,
                                            objDoors[objs[id].specID].stringnum);
                retval = TRUE;
                goto out;
            }
        }
...
access_ok:
    if (DOOR_CLOSED(id) || door_moving(id, TRUE)) {
...
#endif
        {
            ...
            if (((closetime = objDoors[objs[id].specID].autoclose_time) > 0) && (closetime != NEVER_AUTOCLOSE_COOKIE)) {
                ushort new_code;
    ...
    if (play_fx) {
        ...
        case CYBGENFORCE_TRIPLE:
            sfx_id = SFX_DOOR_GRATING;
            break;
        default:
            sfx_id = SFX_DOOR_NORMAL;
            break;
        }
        if (sfx_id != -1)
    ...
    if (((other = objDoors[objs[id].specID].other_half) != OBJ_NULL) && !(in_inv & 0x1)) {
        ...
        if (!otherdoor || (door_moving(id, TRUE) != (DOOR_REALLY_CLOSED(other) || door_moving(other, TRUE))))
    ...
    if (global_fullmap->cyber) {
        ...
        case CYBERHEAL_TRIPLE:
            if (player_struct.cspace_hp != PLAYER_MAX_HP) {
                player_struct.cspace_hp =
                    lg_min(player_struct.cspace_hp + CYBERHEAL_QUANTITY + objSmallstuffs[objs[id].specID].data1,
                           PLAYER_MAX_HP);
        ...
        case CSPACE_EXIT_TRIPLE:
            player_struct.cspace_time_base =
                lg_max(CSPACE_MIN_TIME, player_struct.cspace_time_base - CSPACE_EXIT_PENALTY);
            ...
            default: {
                ...
                if (access_okay) {
                    if (comparator_check(objFixtures[objs[id].specID].comparator, id, &special)) {
                        ...
                        case BUTTON1_TRIPLE:
                        case BUTTON2_TRIPLE:
                            play_digi_fx_obj(SFX_BUTTON, 1, id);
                            break;
                        case BIGRED_TRIPLE:
                            play_digi_fx_obj(SFX_BIGBUTTON, 1, id);
                            break;
                        case BIGLEVER_TRIPLE:
                            play_digi_fx_obj(SFX_BIGLEVER, 1, id);
                            break;
                        case LEVER1_TRIPLE:
                        case LEVER2_TRIPLE:
                            play_digi_fx_obj(SFX_NORMAL_LEVER, 1, id);
                            break;
                        case SWITCH1_TRIPLE:
                        case SWITCH2_TRIPLE:
                            play_digi_fx_obj(SFX_MECH_BUTTON, 1, id);
                            break;
                        }
                        obj_fixture_zoom(id, in_inv, &retval);
            ...
            if (rv) {
                if (pbigs->data2 & 0xFFFF)
                    player_struct.hit_points =
                        lg_min(PLAYER_MAX_HP, player_struct.hit_points + (pbigs->data2 & 0xFFFF));
                else
                    player_struct.hit_points = PLAYER_MAX_HP;
                chg_set_flg(VITALS_UPDATE);
...
errtype elevator_janitor_run() {
    ...
    for (i = 0; i < obj_count; i++) {
        ...
        newloc.z =
            obj_floor_compute(objlist[i], me_height_flr(MAP_GET_XY(OBJ_LOC_BIN_X(dump_loc), OBJ_LOC_BIN_Y(dump_loc))));
...
uchar elevator_use(short dest_level, ubyte which_panel) {
    ...
    case 0:
    case 1:
        new_panel = objFixtures[objs[player_struct.panel_ref].specID].p1;
        break;
    case 2:
    case 3:
        new_panel = objFixtures[objs[player_struct.panel_ref].specID].p2;
        break;
    case 4:
    case 5:
        new_panel = objFixtures[objs[player_struct.panel_ref].specID].p3;
        break;
    }
    if ((which_panel % 2) == 0)
    ...
    if (retval == OK) {
        ...
        if (DOOR_REALLY_CLOSED(id) && !door_locked(id) && objDoors[objs[id].specID].other_half == 0) {
...
errtype keypad_trigger(ObjID id, uchar digits[MAX_KEYPAD_DIGITS]) {
    ...
    if (obj_keypad_crunch(objFixtures[osid].p1, digits))
        match = 1;
    if (obj_keypad_crunch(objFixtures[osid].p2, digits))
        match = 1;
    if (obj_keypad_crunch(objFixtures[osid].p3, digits))
        match = 1;
    if (match == -1) {
