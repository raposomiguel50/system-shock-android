...
#ifdef __ANDROID__
extern void rp5np_aux_audio_pause(void);
extern void rp5np_aux_audio_resume(void);
#else
...
void cutscene_exit(void)
{
  ...
  {
#ifdef __ANDROID__
    /* Stop the SDL_mixer callback while the shared auxiliary stream pointer
       is destroyed. The physical audio device remains owned by SDL_mixer. */
    rp5np_aux_audio_pause();
#else
...
#ifdef __ANDROID__
    rp5np_aux_audio_resume();
    SDL_Log("RP5NP_CUTSCENE_AUDIO_STREAM=CLOSED");
#endif
  }
...
#ifdef __ANDROID__
    rp5np_aux_audio_resume();
    SDL_Log("RP5NP_CUTSCENE_AUDIO_STREAM=OPEN rate=%d", fix_int(amovie->a.sampleRate));
#endif
  }
