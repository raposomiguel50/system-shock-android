...
#define IN_BETWEEN_NOT_EQUAL(a, b, c) ((((a) <= (b)) && ((b) < (c))) || (((a) >= (b)) && ((b) > (c))))
...
ObjID explosion_ray_cast_attack(ObjID gren_id, ObjID target, ObjLoc *gren_loc, fix radius, fix mass, fix speed) {
    ...
    floor_height = obj_height_from_fix(
        fix_from_map_height(me_height_flr(MAP_GET_XY(OBJ_LOC_BIN_X(*gren_loc), OBJ_LOC_BIN_Y(*gren_loc)))));
    ...
    affected_object = ray_cast_vector(gren_id, &source, vector, mass, RAYCAST_ATTACK_SIZE, speed, radius);
            ...
            while (current_ref != OBJ_REF_NULL) {
                ...
                if (!no_effect && affect) {
                    ...
                    if (dist_squared < radius_squared) {
                        ...
                        affected_object = explosion_ray_cast_attack(exclusion, current_id, &hit_loc, edata->radius,
                                                                    edata->knock_mass, NO_RAYCAST_KICKBACK_SPEED);
                        ...
                        if (affected_object == current_id) {
                            ...
                            if (obj_dist < edata->radius_change) {
                                ...
                                ratio =
                                    fix_div((obj_dist - edata->radius_change), (edata->radius - edata->radius_change));
                                damage_fix = (fix_make(1, 0) - ratio) * dmgc;
                            }
                            percent_damage = fix_int(fix_div(damage_fix, max_damage) * 100);

                            effect_class = (objs[current_id].obclass == CLASS_CRITTER)
                                               ? CritterProps[CPNUM(current_id)].hit_effect
                                               : NON_CRITTER_EFFECT;
                            ...
                            if (!player_struct.difficulty[COMBAT_DIFF_INDEX] && (current_id != PLAYER_OBJ))
                                damage =
                                    (ObjProps[OPNUM(current_id)].toughness != 3) ? objs[current_id].info.current_hp : 0;
                            else
                                damage = compute_damage(current_id, edata->dtype, edata->damage_mod, edata->offense,
                                                        edata->penet, percent_damage, NULL, NULL, GREN_TYPE);
                            ...
                            if (damage > 0) {
                                ...
                                if (objs[current_id].obclass == CLASS_GRENADE) {
                                    ...
                                    targ_triple = MAKETRIP(objs[current_id].obclass, objs[current_id].subclass,
                                                           objs[current_id].info.type);
                                ...
                                if (current_id == PLAYER_OBJ) {
                                    ...
                                    if ((loc.x == objs[current_id].loc.x) && (loc.y == objs[current_id].loc.y)) {
                                        message_info(get_temp_string(
                                            REF_STR_HoldingGrenade)); //"Holding a live grenade = Double damage\n");
                                ...
                                if (explosion_affected) {
                                    objGrenades[objs[current_id].specID].unique_id =
                                        0; // get rid of timer - if grenade was timer
                                    ...
                                    if (do_effect) {
                                        ...
                                        if (ObjProps[OPNUM(current_id)].physics_model != 2)
                                            loc.z += obj_height_from_fix(
                                                fix_make(ObjProps[OPNUM(current_id)].physics_xr, 0) /
                                                PHYSICS_RADIUS_UNIT);
