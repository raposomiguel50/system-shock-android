...
static bool map_scroll_clicked = 0;
...
void fsmap_startup(void) {
    ...
    gr_init_sub_canvas(FULLMAP_CANVAS, &fsmap_actual, AMAP_LFT(grd_bm.w), AMAP_TOP(grd_bm.h), AMAP_WID(grd_bm.w),
                       AMAP_HGT(grd_bm.h));
    gr_init_sub_canvas(FULLMAP_CANVAS, &fsmap_bregion, AMAP_RGT(grd_bm.w) + AMAP_BORDER, AMAP_TOP(grd_bm.h),
                       AMAP_BUTTON_WIDTH - 2 * AMAP_BORDER, AMAP_HGT(grd_bm.h));
...
void fsmap_draw_map(void) {
    ...
    f->bm.bits = (uchar *)(f + 1);
    gr_scale_bitmap(&f->bm, (grd_bm.w - (f->bm.w * 2)) / 2, (grd_bm.h - (f->bm.h * 2)) / 2, f->bm.w * 2, f->bm.h * 2);
...
int s_bf(int btn_id, int todo) {
    ...
    switch (todo) {
    case AMAP_TOGGLE:
        fsmap_buttons = (fsmap_buttons & ~fullmask) + ((fsmap_buttons & fullmask) ^ prtlmask);
        break; // xor on/off
    case AMAP_SET:
        fsmap_buttons = (fsmap_buttons & ~fullmask) + prtlmask;
        break; // set on/off to on
    case AMAP_UNSET:
        fsmap_buttons = (fsmap_buttons & ~fullmask);
        break; // set on/off to off
    case BTN_TALK:
        return fsmap_buttons & prtlmask; // return whether this is on or off
    case BTN_PEND:
        break;
    }
...
uchar amap_scroll_handler(uiEvent *ev, LGRegion *reg, intptr_t v) {
    ...
    if (elapsed < (CIT_CYCLE / SCROLL_FRATE))
        return TRUE;
    if (elapsed > 10) elapsed = 10; //prevent jumping of map
    map_scrolltime = now;
    if (map_scroll_clicked) elapsed *= 10; //clicking nswe acts like a longer elapsed time
        ...
        if ((deal_data = amap_deal_with_map_click(amptr, &x, &y)) != NULL) {
            ...
            if (amptr->note_obj == OBJ_NULL) {
                ...
                if ((amptr->note_obj = object_place(MAPNOTE_TRIPLE, MakePoint(x, y))) != OBJ_NULL)
                    ...
                    last_msg_ok = FALSE;
                    clear_cur_mapnote();
                    WARN("%s: No more mapnote space", __FUNCTION__);
                }
            }
