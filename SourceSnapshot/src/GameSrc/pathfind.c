...
#define SET_PATH_STEP(pathid, stepnum, newval)                                                                        \
    do {                                                                                                              \
        PATH_CHAR(pathid, stepnum) =                                                                                  \
...
errtype find_path(char path_id) {
    ...
    while (!done && step_count < NUM_PATH_STEPS) {
        ...
        // If we HAVE found it, go poke in the right info into the path
        {
            ...
            while (!PointsEqual(currpt, paths[path_id].source)) {
                ...
                case 0:
                    currpt.y--;
                    break; // N, so backwards is S
                case 1:
                    currpt.x--;
                    break; // E, so backwards is W
                case 2:
                    currpt.y++;
                    break; // S, so backwards is N
                case 3:
                    currpt.x++;
                    break; // W, so backwards is E
                }
            }
    ...
    if (step_count >= NUM_PATH_STEPS) {
        paths[path_id].num_steps = 0;
        //      Warning(("Failed to find path from (%x,%x) to
