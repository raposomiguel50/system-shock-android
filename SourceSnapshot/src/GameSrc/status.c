...
uchar under_bio(int x) {
    if (((x >= 16) && (x <= 22)) || ((x >= 69) && (x <= 74)) || ((x >= 114) && (x <= 208)) ||
        ((x >= 252) && (x <= 258)) || ((x >= 298) && (x <= 303)))
        return (TRUE);
    return (FALSE);
}
...
uchar status_track_free(int track) { return bio_data[track].free; }

uchar status_track_active(int track) { return bio_data[track].active; }

void status_track_activate(int track, uchar active) { bio_data[track].active = active; }
            ...
            if (curr_blk->counter < curr_blk->update_time) {
                ...
                if (curr_blk->tail == FALSE) {
                    ...
                    for (j = 0, draw_location = the_head - curr_blk->color_length;
                         j < the_head / curr_blk->color_length; j++, draw_location -= curr_blk->color_length) {
                    ...
                    for (j = 0, draw_location = FIND_OVERLAP(the_head, curr_blk->color_length); j < (COLOR_CHANGES - 1);
                         j++, draw_location = FIND_OVERLAP(draw_location, curr_blk->color_length)) {
                        color = curr_blk->height[draw_location] & COLOR_BIO_MASK;
                        if ((color != COLOR_BIT_SHIFT(j + 1)) || (rand() > (RAND_MAX / 2))) {
                            curr_blk->height[draw_location] &= HEIGHT_BIO_MASK;
                            curr_blk->height[draw_location] |= COLOR_BIT_SHIFT(j + 2);
...
void draw_bio_height(int track_number, int draw_location) {
    ...
    if ((abs(height - prevHeight) < SPIKE_THRESHOLD) || (curr_blk->special & NO_SPIKE) ||
