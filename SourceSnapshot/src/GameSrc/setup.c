...
#ifdef __ANDROID__
static bool rp5np_name_text_input_active = false;
static bool rp5np_name_keyboard_shown_logged = false;

static void rp5np_name_text_commit(const char *utf8_text, void *userdata);

static void rp5np_start_name_text_input(void) {
    if (rp5np_name_text_input_active)
        return;

    rp5np_set_text_input_handler(rp5np_name_text_commit, NULL);
    SDL_StartTextInput();
    rp5np_name_text_input_active = (SDL_IsTextInputActive() == SDL_TRUE);
    rp5np_name_keyboard_shown_logged = false;
    SDL_Log("RP5NP_NAME_TEXT_INPUT_START active=%d keyboard_support=%d",
            rp5np_name_text_input_active ? 1 : 0,
            SDL_HasScreenKeyboardSupport() == SDL_TRUE ? 1 : 0);
}

static void rp5np_stop_name_text_input(void) {
    rp5np_set_text_input_handler(NULL, NULL);

    if (!rp5np_name_text_input_active && SDL_IsTextInputActive() != SDL_TRUE) {
        rp5np_name_text_input_active = false;
        rp5np_name_keyboard_shown_logged = false;
        return;
    }
    ...
    rp5np_name_text_input_active = false;
    rp5np_name_keyboard_shown_logged = false;
    SDL_Log("RP5NP_NAME_TEXT_INPUT_STOP");
}

static void rp5np_poll_name_text_input(void) {
    if (!rp5np_name_text_input_active || rp5np_name_keyboard_shown_logged)
        return;

    SDL_Window *focused_window = SDL_GetKeyboardFocus();
    if (focused_window != NULL && SDL_IsScreenKeyboardShown(focused_window) == SDL_TRUE) {
        rp5np_name_keyboard_shown_logged = true;
        SDL_Log("RP5NP_NAME_SCREEN_KEYBOARD_SHOWN=1");
    }
}
#endif
...
static void rp5np_name_text_commit(const char *utf8_text, void *userdata) {
    ...
    if (utf8_text == NULL || setup_mode != SETUP_DIFFICULTY)
        return;

    char old_name[sizeof(player_struct.name)];
    memcpy(old_name, player_struct.name, sizeof(old_name));
    old_name[sizeof(old_name) - 1] = '\0';

    size_t name_len = strlen(player_struct.name);
    unsigned int accepted = 0;
    unsigned int rejected = 0;

    const unsigned char *cursor = (const unsigned char *)utf8_text;
    while (*cursor != '\0') {
        ...
        if (strchr(valid_char_string, (int)ch) == NULL) {
            ++rejected;
            continue;
        }

        if (name_len + 1 >= sizeof(player_struct.name)) {
            ++rejected;
            continue;
        }

        player_struct.name[name_len++] = (char)ch;
        player_struct.name[name_len] = '\0';
        ++accepted;
    }

    if (accepted > 0) {
        draw_username(0, old_name);
        draw_username(NORMAL_ENTRY_COLOR, player_struct.name);
    }

    SDL_Log("RP5NP_NAME_TEXT_COMMIT bytes=%u accepted=%u rejected=%u name_len=%u",
            (unsigned int)strlen(utf8_text),
            accepted,
            rejected,
...
errtype difficulty_draw(uchar full) {
    int i;
#ifdef __ANDROID__
    const bool rp5np_entering_name_screen = (setup_mode != SETUP_DIFFICULTY);
#endif
...
#ifdef __ANDROID__
    if (rp5np_entering_name_screen)
        rp5np_start_name_text_input();
#endif
...
#ifdef __ANDROID__
    rp5np_stop_name_text_input();
    rp5np_controller_set_setup_context(false);
#endif
        ...
        case SETUP_DIFFICULTY:
            ...
            for (i = 0; i < 16; i++) {
                if ((ev->pos.x > (build_diff_x(i) - 2)) && (ev->pos.x < (build_diff_x(i) - 2 + DIFF_SIZE_X)) &&
                    ...
                    TRACE("%s: difficulty %d set to %d (verify = %d)", __FUNCTION__, i / 4, i % 4,
                          player_struct.difficulty[i / 4]);
...
#ifdef __ANDROID__
    rp5np_stop_name_text_input();
    rp5np_controller_set_setup_context(true);
#endif
