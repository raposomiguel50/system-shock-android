                ...
                while (oref != OBJ_REF_NULL) {
                    ...
                    if (id != PLAYER_OBJ) {
                        ...
                        case CLASS_CRITTER:
                            if (music_on) {
                                if ((new_monster == -1) && (dist < best_dist)) {
                                    ...
                                    if (pf_id != -1) {
                                        check_requests(TRUE);
                                        if ((paths[pf_id].num_steps != 0) &&
                                            (paths[pf_id].num_steps <= MONSTER_THEME_PATH_LENGTH)) {
                                            delete_path(pf_id);
                                            pf_id = -1;
                                        }
                                    }
                                ...
                                if ((dist < PERIL_RADIUS) && (objCritters[osid].mood != AI_MOOD_FRIENDLY) &&
                                    (objCritters[osid].orders != AI_ORDERS_SLEEP)) {
                                    mlimbs_peril = DEFAULT_PERIL_MAX;
                                }
                            }
                            if ((dist < NEAR_NOISE_RADIUS) && (CritterProps[CPTRIP(trip)].near_sound != 255) &&
                        ...
                        case SPARK_CABLE_TRIPLE:
                            if ((dist < SPARKING_RADIUS) &&
                                ((rand() & STOCHASTIC_SPARKING_MASK) < STOCHASTIC_SPARKING_LEVEL))
...
void fatigue_player(void) {
    ...
    if (gamesys_fatigue && !jumpjets_active && !EDMS_pelvis_is_climbing()) {
        ...
        if (player_struct.fatigue > MAX_FATIGUE)
...
uchar shodan_phase_in(uchar *bitmask, short x, short y, short w, short h, short num, uchar dir) {
    ...
    while (i < num) {
        ...
        if (dir) {
            ...
            while (!SHODAN_CONQUER_GET(bitmask, val) && (val < SHODAN_BITMASK_SIZE) &&
                   (val - oval <= UNCONQUER_THRESHOLD))
                val++;
            if (val < SHODAN_BITMASK_SIZE)
            ...
            if (thresh_fail) {
                ...
                for (i = 0; i < NUM_SHODAN_REGIONS; i++) {
                    shodan_phase_in(shodan_bitmask, shodan_region_full_x[i], shodan_region_full_y[i],
                                    shodan_region_full_width[i], shodan_region_full_height[i],
                                    QUESTVAR_GET(CYBER_DIFF_QVAR) + 1, TRUE);
...
void do_stuff_every_second() {
    ...
    if (running_dt > SECOND_UPDATE_FREQ) {
        if (global_fullmap->cyber) {
            ...
            if (next < last)
                next += HEALTH_RESTORE_UNIT;
            if (player_struct.energy_regen + player_struct.energy_spend != 0) {
                int num = player_struct.energy_regen - player_struct.energy_spend;
                if (num != 0) {
            ...
            if (player_struct.hit_points < PLAYER_MAX_HP && player_struct.hit_points_regen != 0) {

                int num = player_struct.hit_points_regen;
                player_struct.hit_points = apply_rate(player_struct.hit_points, num, last, next, 0, PLAYER_MAX_HP);
        ...
        if (player_struct.fatigue > 0 && player_struct.controls[CONTROL_YVEL] <= SPRINT_CONTROL_THRESHOLD &&
            ...
            player_struct.fatigue_regen = player_struct.fatigue_regen_base;
        ...
        // what is this code trying to do? ie. r_a -= (0-r_a)-r_a/4; looks like it can increase or decrease?, end <0, so
        // on
        if (rad_absorb > 0)
            rad_absorb -= rand() % (rad_absorb)-rad_absorb / 4;
        else
            rad_absorb = 0;
        if (bio_absorb > 0)
            bio_absorb -= rand() % (bio_absorb)-bio_absorb / 4;
        else
            bio_absorb = 0;
...
int enviro_suit_absorb(int damage, int exposure, ubyte dtype) {
    ...
    if (dtype == RADIATION_TYPE && version > 0)
        version--;
    if (version == 0)
        return damage;
