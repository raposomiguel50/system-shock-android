...
int mlimbs_load_theme(char *xname, char *xinfo, int thmid) {
    ...
    fread(&num_XMIDI_sequences, sizeof(uchar), 1,
...
int mlimbs_start_theme(void) {
    if ((mlimbs_status == 0) || (mlimbs_theme == NULL))
        return -1; // no can do
    if (mlimbs_master_slot >= 0)
        return 1; // already can done

    if ((mlimbs_master_slot = snd_sequence_play(mrefBuild(mlimbs_cur_theme_id, CALLBACK_SEQ_NUM), mlimbs_theme,
                                                CALLBACK_SEQ_NUM, NULL)) ==
...
void mlimbs_mute_sequence_channel(int usernum, int x, uchar mute) {
...
#endif
            if ((val < 0) || (val > 127)) {
                AIL_send_channel_voice_message(NULL, _uiD_seq(usernum), MIDI_CONTROL_CHANGE | (seq_ch - 1),
                                               XMIDI_BANK_SELECT, 0);
            }
    ...
    userID[usernum].sequence_channel_status[x - 10] =
        (mute == TRUE) ? SEQUENCE_CHANNEL_MUTED : SEQUENCE_CHANNEL_PENDING;
...
int mlimbs_channel_prioritize(int priority, int pieceID, int voices_needed, uchar crossfade, uchar channel_prioritize) {
    ...
    if (crossfade != TRUE || channel_prioritize == FALSE) {
        ...
        channels_needed = 0; // If we're crossfading, mlimbs_channel_prioritize
...
int mlimbs_assign_channels(int usernum, uchar crossfade) {
    ...
    if (c_map == 0)
        return 1; // If no channels are requested from dynamic channel allocation, return.
    for (j = 11; j < 17; j++) {
        if ((c_map >> (j - 1)) & 0x0001) { // Mark the sequence channel as waiting for a physical channel
...
int mlimbs_play_piece(int pieceID, int priority, int loops, int rel_vol, uchar channel_prioritize, uchar crossfade) {
    ...
    // priority,loops,rel_vol,channel_prioritize,crossfade));
    ...
    if (slot >= 0) {
        ...
        if (cpl_num < 10)
            curr_play_list[cpl_num++] = pieceID;
        if (mlimbs_assign_channels(usernum, crossfade) == -1) { /* Try and assign the channels and voices freed above */
    ...
    for (i = 0; i < MLIMBS_MAX_SEQUENCES - 1; i++) {
        if (userID[i].pieceID != -1) {
            for (j = 0; j < MLIMBS_MAX_SEQUENCES - 1; j++) {
                if (current_request[j].pieceID == userID[i].pieceID) { // Begin rampout of this chunk, if desired.
                        ...
                        if (current_request[j].crossfade < 0) {
                            ...
                            while (userID[i].crossfade_status > 10) {
                                if (userID[i].sequence_channel_status[userID[i].crossfade_status - 10] >= 0) {
                                    ...
                                    break;
                                } else if (userID[i].sequence_channel_status[userID[i].crossfade_status - 10] ==
                                           SEQUENCE_CHANNEL_PENDING) {
                                    userID[i].sequence_channel_status[userID[i].crossfade_status - 10] =
                                        SEQUENCE_CHANNEL_MUTED;
                                    userID[i].crossfade_status--;
                                    break;
                                }
                                userID[i].crossfade_status--;
                            }
                            // secret_sprint((ss_temp,"crossfade fun for %d (req %d) got
                ...
                if (userID[j].pieceID == current_request[i].pieceID) {
                    if (current_request[i].crossfade > 0) {
                        if (userID[j].crossfade_status < 11)
                            userID[j].crossfade_status = 11;
                        while (userID[j].crossfade_status < 17) {
                            if (userID[j].sequence_channel_status[userID[j].crossfade_status - 10] ==
                                SEQUENCE_CHANNEL_PENDING)
                                break;
                            else if (userID[j].sequence_channel_status[userID[j].crossfade_status - 10] ==
                                     SEQUENCE_CHANNEL_MUTED)
                                userID[j].sequence_channel_status[userID[j].crossfade_status - 10] =
                                    SEQUENCE_CHANNEL_PENDING;
                            else
            ...
            if (j < MLIMBS_MAX_SEQUENCES - 1)
                ...
                if ((mlimbs_counter * num_master_measures) % xseq_info[current_request[i].pieceID].num_measures ==
                    ...
                    if (current_request[i].ramp > 0)
                        ...
                        rvol = current_request[i].rel_vol;
                    usernum = mlimbs_play_piece(current_request[i].pieceID, current_request[i].priority,
                                                current_request[i].loops, rvol, current_request[i].channel_prioritize,
                                                (current_request[i].crossfade > 0));
                    // secret_sprint((ss_temp,"brought in %d [%d req %d] crossfade %d got %d (rv
                    // %d)\n",current_request[i].pieceID,j,i,current_request[i].crossfade,usernum,rvol));
                    if (usernum >= 0) {
                        userID[usernum].rel_vol = current_request[k].rel_vol;

                        if (current_request[i].crossfade > 0)
                            userID[usernum].crossfade_status =
                                11; // If we're crossfading in, crossfade starting with channel 11
                        else
                            userID[usernum].crossfade_status =
