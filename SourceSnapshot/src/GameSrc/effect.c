...
errtype increment_anim(ulong num_units) {
    ...
    for (i = 0; i < anim_counter; i++) {
        ...
        if (animlist[i].flags & ANIMFLAG_REVERSE) {
            switch (objs[id].obclass) {
            case CLASS_DOOR:
                if ((objs[id].info.current_frame - interval < DOOR_OPEN_FRAME) &&
                    (objs[id].info.current_frame >= DOOR_OPEN_FRAME)) {
            ...
            case CLASS_DOOR:
                if (((objs[id].loc.p != 0) || (objs[id].loc.b != 0)) &&
                    (objs[id].info.current_frame < DOOR_OPEN_FRAME) &&
                    (objs[id].info.current_frame + interval >= DOOR_OPEN_FRAME)) {
                    obj_physics_refresh_area(OBJ_LOC_BIN_X(objs[id].loc), OBJ_LOC_BIN_Y(objs[id].loc), TRUE);
                }
                break;
            }
    ...
    while (osid != OBJ_SPEC_NULL) {
        ...
        if ((id != PLAYER_OBJ) && (!ai_critter_sleeping(osid) || (get_crit_posture(osid) == DEATH_CRITTER_POSTURE))) {
            new_units = num_units + objs[id].info.time_remainder;
            ...
            if ((post == ATTACKING_CRITTER_POSTURE) || (post == ATTACKING2_CRITTER_POSTURE)) {
                ...
                att_frame = (CritterProps[CPTRIP(triple)].fire_frame == 0) ? (curr_frames * 3) / 4
                                                                           : CritterProps[CPTRIP(triple)].fire_frame;

                // Only attack when first reaching or passing sancted attack frame
                if ((objs[id].info.current_frame < att_frame) &&
                    (objs[id].info.current_frame + interval >= att_frame)) {
