...
void do_seen_pass(void) {
    ...
    lx = _fr_x_cen - SEEN_SIZE;
    tx = lx + (SEEN_SIZE << 1);
    if (lx < 0)
        lx = 0;
    if (tx >= MAP_XSIZE)
        tx = MAP_XSIZE - 1;
    ly = _fr_y_cen - SEEN_SIZE;
    ty = ly + (SEEN_SIZE << 1);
    if (ly < 0)
        ly = 0;
    if (ty >= MAP_YSIZE)
...
int fr_pipe_go_3(void) {
    ...
    endcaps[0] = center + (fr_map_x * frpipe_dist);
    endcaps[2] = center - (fr_map_x * frpipe_dist);
    endcaps[1] = center + frpipe_dist;
    endcaps[3] = center - frpipe_dist;
    clip_len[0] = _fr_y_cen + _fdt_dist - fr_map_y;
    clip_len[2] = _fdt_dist - _fr_y_cen;
    clip_len[1] = _fr_x_cen + _fdt_dist - fr_map_x;
    clip_len[3] = _fdt_dist - _fr_x_cen;
    ...
    while (_fdt_dist > 0) // move in from the maximal distance
    {
        ...
        if ((_fdt_dist & 1) == 0) // do the perfect diagonal
        {
            ...
            for (i = 0; i < 4; i++) { // go through each quadrant, dealing
                j = quad_order[i];
