...
ObjID ray_cast_objects(ObjID src, ObjID dest, fix bullet_mass, fix bullet_size, fix bullet_speed, fix bullet_range) {
    ...
    ray.origin.z =
        fix_from_obj_height(src) + (fix_make(ObjProps[OPNUM(src)].physics_xr, 0) / (PHYSICS_RADIUS_UNIT << 1));
    ...
    if (objs[dest].info.ph != -1) {
        ...
        target_loc.x = new_state.X;
        target_loc.y = new_state.Y;
        target_loc.z = new_state.Z;
    } else {
        ...
        target_loc.z =
            fix_from_obj_height(dest) + (fix_make(ObjProps[OPNUM(src)].physics_xr, 0) / (PHYSICS_RADIUS_UNIT << 1));
    }
...
void find_fire_vector(LGPoint *pt, Combat_Pt *vector) {
    ...
    x1 = main_view_vectors[2].gX + (fix_mul((main_view_vectors[1].gX - main_view_vectors[2].gX), fix_make(x, 0))) /
                                       ((fauxrend_context *)_current_fr_context)->xwid;
    x2 = main_view_vectors[3].gX + (fix_mul((main_view_vectors[0].gX - main_view_vectors[3].gX), fix_make(x, 0))) /
                                       ((fauxrend_context *)_current_fr_context)->xwid;

    // negative because we're changing coordinate frames
    y1 = -main_view_vectors[2].gY + (fix_mul((main_view_vectors[2].gY - main_view_vectors[1].gY), fix_make(x, 0))) /
                                        ((fauxrend_context *)_current_fr_context)->xwid;
    y2 = -main_view_vectors[3].gY + (fix_mul((main_view_vectors[3].gY - main_view_vectors[0].gY), fix_make(x, 0))) /
                                        ((fauxrend_context *)_current_fr_context)->xwid;

    z1 = main_view_vectors[2].gZ + (fix_mul((main_view_vectors[1].gZ - main_view_vectors[2].gZ), fix_make(x, 0))) /
                                       ((fauxrend_context *)_current_fr_context)->xwid;
    z2 = main_view_vectors[3].gZ + (fix_mul((main_view_vectors[0].gZ - main_view_vectors[3].gZ), fix_make(x, 0))) /
                                       ((fauxrend_context *)_current_fr_context)->xwid;
