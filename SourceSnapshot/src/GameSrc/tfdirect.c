...
// i love us
uchar v_to_cur[] = {
    (SS_BCD_CURR_E | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,    (SS_BCD_CURR_W | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,
    (SS_BCD_CURR_N | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,    (SS_BCD_CURR_S | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,
    (SS_BCD_CURR_E | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,    (SS_BCD_CURR_W | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,
    (SS_BCD_CURR_N | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,    (SS_BCD_CURR_S | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,
    (SS_BCD_CURR_E | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF,   (SS_BCD_CURR_W | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF,
    (SS_BCD_CURR_N | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF,   (SS_BCD_CURR_S | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF,
    (SS_BCD_REPUL_UP | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,  (SS_BCD_REPUL_DOWN | SS_BCD_CURR_LOW) >> SS_BCD_CURR_SHF,
    (SS_BCD_REPUL_UP | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,  (SS_BCD_REPUL_DOWN | SS_BCD_CURR_MID) >> SS_BCD_CURR_SHF,
    (SS_BCD_REPUL_UP | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF, (SS_BCD_REPUL_DOWN | SS_BCD_CURR_HIGH) >> SS_BCD_CURR_SHF
};
...
i_hate_everyone : {
    ...
    which = ((flags & SS_BCD_TYPE_MASK) == SS_BCD_TYPE_WALL)
                ? 2
                : (((flags & SS_BCD_TYPE_MASK) == SS_BCD_TYPE_CEIL) ? 0 : 1);
...
#define set_n_chk_1(val)                              \
    _1d_pt[1] = (val);                                \
    if ((_1d_pt[1] > 0) || (_1d_pt[1] < -tf_cur_rad)) \
    continue
...
fix _tf_border_check_2d(void) {
    ...
    for (i = 0; i < tf_pcnt; i++) {
        if ((tf_norm_2d[i][0] != 0) &&
            (tf_norm_2d[i][1] != 0)) { // do the real case, have to learn about cross clock, i think
            fix nfac, c_pt[2];
            tf_Spew(BordChk,
                    ("gc2d: face %d fixing to solve real case w %x %x\n", i, tf_norm_2d[i][0], tf_norm_2d[i][1]));
            c_pt[0] = tf_pt[0] - tf_vert_2d[i][0];
            c_pt[1] = tf_pt[1] - tf_vert_2d[i][1];
            _1d_pt[1] = fix_mul(tf_norm_2d[i][0], c_pt[0]) + fix_mul(tf_norm_2d[i][1], c_pt[1]);
            if (_1d_pt[1] >= 0)
                continue; // behind the edge, here we can only check behind, as we are unnormalized as of yet
            ...
            else {                 // finish setting up the 1d case
        ...
        } else // is clock gnosis correct or not? who the hell knows...
        {
            ...
            if (tf_norm_2d[i][0] == 0)    // north/south normal
                ...
                {
                    set_n_chk_1(tf_pt[1] - tf_vert_2d[i][1]);
                    _1d_pt[0] = tf_vert_2d[i][0] - tf_pt[0];
                    _1d_endpt[0] = tf_norm_2d[i][1];
                } else // south facing normal, ie. invert everything, perhaps this could be cooler, eh?
                {
                    set_n_chk_1(tf_vert_2d[i][1] - tf_pt[1]);
                    _1d_pt[0] = tf_pt[0] - tf_vert_2d[i][0];
                    _1d_endpt[0] = -tf_norm_2d[i][1];
                }
            else                          // east/west normal
                if (tf_norm_2d[i][0] > 0) // this is the south facing edge, east facing interior normal
            {
                set_n_chk_1(tf_pt[0] - tf_vert_2d[i][0]);
                _1d_pt[0] = tf_pt[1] - tf_vert_2d[i][1];
                _1d_endpt[0] = tf_norm_2d[i][0];
            } else {
                set_n_chk_1(tf_vert_2d[i][0] - tf_pt[0]);
                _1d_pt[0] = tf_vert_2d[i][1] - tf_pt[1];
                _1d_endpt[0] = -tf_norm_2d[i][0];
            }
        }
...
fix tf_solve_2d_case(int flags) {
    ...
    if (flags & TF_FLG_BOX_MASK) {
        ...
        {                            // do box internal/external check, set flag
            ...
            if (xd < 0)
                xd = tf_pt[0] - tf_vert_2d[2][0]; // if not, right?
            if (xd < 0)
                xd = 0;                       // in middle
...
parse_ichk:
    ...
    case TF_FLG_ICHK_NONE:
        WARN("%s: tfd: no ichk data", __FUNCTION__);
...
uchar tf_solve_aligned_face(fix pt[3], fix walls[4][2], int flags, fix *norm) {
    ...
    //      tf_turn_on(0xffff);
    tf_Spew(AlignFce,
            ("tfd:aligned walls %x %x %x %x %x %x %x %x, pt %x %x %x, flg %x, nrm %x %x %x\n", walls[0][0], walls[0][1],
             walls[1][0], walls[1][1], walls[2][0], walls[2][1], walls[3][0], walls[3][1], pt[0], pt[1], pt[2], flags,
             norm != NULL ? norm[0] : 0xb, norm != NULL ? norm[1] : 0xa, norm != NULL ? norm[2] : 0xd));
...
uchar tf_solve_remetriced_face(fix pt[3], fix walls[4][2], int flags, fix norm[3], fix metric) {
    ...
    tf_Spew(RemetFce, ("tfd:remetric walls %x %x %x %x %x %x %x %x, pt %x %x %x, nrm %x %x %x, flg %x, metric %x\n",
                       walls[0][0], walls[0][1], walls[1][0], walls[1][1], walls[2][0], walls[2][1], walls[3][0],
                       walls[3][1], pt[0], pt[1], pt[2], norm[0], norm[1], norm[2], flags, metric));
    tf_pt = pt;
    tf_vert_2d = walls;
...
uchar tf_solve_cylinder(fix pt[3], fix irad, fix height) {
    ...
    // first check height
    if ((pt[2] < -tf_rad) || (pt[2] > height + tf_rad))
        return rv; // nope, high or low
        ...
        if ((r_dist = fix_sqrt(dist_sqrd)) < urad) {
            ...
            if ((pt[2] < 0) || (pt[2] > height)) // flat top+bottom
            {
                ...
                if (irad > 0) {
                    ...
                    if (nrm[2] > 0)
                        flags = SS_BCD_PRIM_ZAXIS | SS_BCD_TYPE_FLOOR;
                    else
                        flags = SS_BCD_PRIM_NEG_Z | SS_BCD_TYPE_CEIL;
...
void terrfunc_one_map_square(int fmask) { // add appropriately set facelet_mask appropriately
...
TerrainHit tf_direct(fix fix_x, fix fix_y, fix fix_z, fix rad, int32_t ph, TFType tf_type) {
    ...
    maxx = fix_int(fix_x + rad);
    xd = maxx - minx + 1 + xo; // get unclipped x width/distance stuff
    if (maxx >= MAP_XSIZE)
        maxx = MAP_XSIZE - 1;
    maxy = fix_int(fix_y + rad);
    yd = maxy - miny + 1 + yo; // get unclipped y width/distance stuff
    if (maxy >= MAP_YSIZE)
        maxy = MAP_YSIZE - 1;
    tfunc_maxz = fix_z + rad;
    tfunc_minz = fix_z - rad; // set top and bottom height for square
    ...
    {
        tfunc_map_x = minx;
        tfunc_map_y = miny; // do we really need this?
        ...
        xb = tf_loc_pt[0];
        fce_minc = MAP_XSIZE - (maxx - minx) - 1;
