    ...
    for (ObjID id = (objs[OBJ_NULL]).headused; id != OBJ_NULL; id = objs[id].next) {
        ...
        if (ICE_ICE_BABY(id) && ICE_AGIT(id) && ((rand() & 0xFF) < ice_fire_chances[ICE_LEVEL(id)])) {
            ...
            if (dist < AGITATED_ICE_DIST) {
                ...
                ai_fire_special(id, PLAYER_OBJ, CYBERBOLT_TRIPLE, objs[id].loc, objs[PLAYER_OBJ].loc, ICE_LEVEL(id),
                                SLOW_PROJECTILE_DURATION);
            }
        }
    }
...
short compute_base_visibility() {
    ...
    if (!cspace_decoy_obj) {
        ...
        visibility += (LIGHT_VISIBLE_FACTOR / lg_max(1, (lg_max(0, me_light_flr(pme) - me_templight_flr(pme)) +
                                                         lg_max(0, me_light_ceil(pme) - me_templight_ceil(pme))) /
                                                            2));
        ...
        if (player_struct.last_fire) { // make sure we've actually fired a weapon
            visibility +=
                ((GUN_VISIBLE_FACTOR * CIT_CYCLE) / lg_max(player_struct.game_time - player_struct.last_fire, 1));
        }
...
errtype run_evil_otto(ObjID id, int dist) {
    ...
    if (dist < SHODAN_AVATAR_HOSAGE_DISTANCE) {
        ...
        //      attack_object(PLAYER_OBJ, CritterProps[cp_num].attacks[0].damage_type,
        //      CritterProps[cp_num].attacks[0].damage_modifier,
        //         CritterProps[cp_num].attacks[0].offense_value, CritterProps[cp_num].attacks[0].penetration, 0, 100,
        //         NULL, 0, 0, NULL);
    } else {
        fix xvec, yvec, fdist;
        int dx, dy;
...
uchar do_physics_stupidity(ObjID id, int big_dist) {
    ...
    if ((objCritters[osid].path_id != -1) || (objCritters[osid].mood == AI_MOOD_HOSTILE) ||
        ...
        if (!EDMS_frere_jaques(objs[id].info.ph)) {
            //         Spew(DSRC_PHYSICS_Sleeper, ("obj id %x, ph = %d(0x%x) is PF or attack but
    ...
    if (dist > use_dist) {
        ...
        //      if (EDMS_frere_jaques(objs[id].info.ph))
        //         Spew(DSRC_PHYSICS_Sleeper, ("obj id %x, ph = %d(0x%x) is too far but
    ...
    if (cspace_decoy_obj) {
        ...
        *raycast_success = TRUE;
        ...
        //      else
        {
            ...
            ang_diff =
                point_in_view_arc(plr_state.X, plr_state.Y, our_state.X, our_state.Y,
                                  0x4000 - fixang_from_phys_angle(phys_angle_from_obj(objs[id].loc.h)), &real_ang);
        ...
        if (use_vis > 0) {
            ...
            r = rand() % 255;
            if (r < use_vis) {
                if (ray_cast_objects(id, PLAYER_OBJ, VISIBLE_MASS, VISIBLE_SIZE, VISIBLE_SPEED, VISIBLE_RANGE) ==
                    PLAYER_OBJ)
...
errtype follow_pathfinding(ObjID id, ObjSpecID osid) {
    ...
    if ((objCritters[osid].pf_x == -1) || (paths[path_id].curr_step == -1) ||
        ...
        if (!(((ObjProps[OPNUM(id)].flags & CLASS_FLAGS) >> CLASS_FLAGS_SHF) & CRITTER_NODOOR_OBJPROP_FLAG)) {
            ...
            if ((abs(csq.x - sq.x) < 2) && (abs(csq.y - sq.y) < 2))
                pf_obj_doors(MAP_GET_XY(csq.x, csq.y), MAP_GET_XY(sq.x, sq.y),
                             dir_table[sq.y - csq.y + 1][sq.x - csq.x + 1], &open_me);
...
errtype run_combat_ai(ObjID id, uchar raycast_success) {
    ...
    if (pcrit->mood == AI_MOOD_ATTACKING) {
        ...
        if ((get_crit_posture(osid) != ATTACKING_CRITTER_POSTURE) &&
            (get_crit_posture(osid) != ATTACKING2_CRITTER_POSTURE)) {
            if (ai_ranges) {
                if (pcrit->attack_count < player_struct.game_time) {
                    ...
                    if (!(ai_ranges & 0x1))
                        ...
                        posture = (rand() % 255 < CritterProps[cp_num].alt_perc) ? ATTACKING2_CRITTER_POSTURE
                                                                                 : ATTACKING_CRITTER_POSTURE;
    ...
    if (raycast_success) {
        ...
        if ((ai_ranges) && (!(objs[id].info.inst_flags & CLASS_INST_FLAG2)) && (QUESTVAR_GET(COMBAT_DIFF_QVAR) < 3) &&
            (ID2TRIP(id) != AUTOBOMB_TRIPLE)) {
            objCritters[osid].des_speed = DEFAULT_SPEED >> 3;
...
LGPoint ai_highway_func(ObjID oid, ObjSpecID osid) {
    ...
    case 0:
        param = objTraps[objs[curr_id].specID].p1;
        break;
    case 1:
        param = objTraps[objs[curr_id].specID].p2;
        break;
    case 2:
        param = objTraps[objs[curr_id].specID].p3;
        break;
    }
    ...
    if ((objTraps[objs[curr_id].specID].p4 & 0xFFFF) != 0) {
        ...
        case 1:
            do_multi_stuff(interface_param);
            break;
        }
    }
...
// some other way
LGPoint (*ai_order_funcs[])(ObjSpecID, ObjID) = {ai_none_func,   ai_roam_func,    ai_none_func,
        ...
        if (do_physics_stupidity(id, big_dist)) {
            ...
            if (ai_order_funcs[objCritters[osid].orders] != NULL) {
                ...
                if (dest.x != -1) {
                    ...
                    //               Spew(DSRC_AI_Path, ("peace(%x): id %x getting new path (%d), from %x,%x to
                    //               %x,%x\n",
...
errtype ai_run() {
    ...
    while (osid != OBJ_SPEC_NULL) {
        ...
        if ((objCritters[osid].orders == AI_ORDERS_SLEEP) || (get_crit_posture(osid) == DEATH_CRITTER_POSTURE))
            goto ai_loop_end;
        ...
        dist = long_fast_pyth_dist(objs[id].loc.x - objs[PLAYER_OBJ].loc.x, objs[id].loc.y - objs[PLAYER_OBJ].loc.y);
        ...
        if (objCritters[osid].wait_frames <= 0) {
            ...
            if (((mood == AI_MOOD_HOSTILE) || (mood == AI_MOOD_ATTACKING)) &&
                !(objCritters[osid].flags & AI_FLAG_CONFUSED)) {
