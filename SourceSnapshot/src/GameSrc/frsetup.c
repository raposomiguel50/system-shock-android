...
void fr_set_cluts(uchar *base, uchar *bwclut, uchar *greenclut, uchar *amberclut) {
    ...
    _fr_clut_list[1] = bwclut;
    _fr_clut_list[2] = greenclut;
    _fr_clut_list[3] = amberclut;
}
...
frc *fr_place_view(frc *view, void *v_cam, void *cnvs, int pflags, char axis, int fov, int xc, int yc, int wid,
    ...
    */
    ...
    fr->draw_call = _fr_glob_draw_call;
    fr->horizon_call = _fr_glob_horizon_call;
    fr->render_call = _fr_glob_render_call;
    fr->draw_canvas = fr->main_canvas;
    fr->axis = axis;
    fr->fov = fov;
    fr->xtop = xc;
    fr->ytop = yc;
...
void _fr_update_context(int det) {
    if (_fr->flags & FR_DOUBLEB_MASK)
        ...
        gr_init_sub_canvas(&_fr->main_canvas, &_fr->draw_canvas, 0, 0, _fr->xwid >> det_sizing[det][0],
                           _fr->ywid >> det_sizing[det][1]);
    _fr->last_detail = det;
}
...
void _fr_change_detail(int det) {
    ...
    // mprintf("Tmpz %x, vz %x (%d <- %d), ps. %x\n",tmpz,_fr->viewer_zoom,_fr->detail,_fr_last_detail,*(fix
    // *)((&scrw)-5));
    _fr_last_detail = det;
}
