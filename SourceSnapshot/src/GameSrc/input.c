...
#else
        ui_mouse_constrain_xy(view->abs_x, view->abs_y, view->abs_x + RectWidth(view->r) - 1,
                              view->abs_y + RectHeight(view->r) - 1);
#endif
    }
}
...
void FilterSpaceballDataFloating(long *tx, long *ty, long *tz, long *rx, long *ry, long *rz) {
    ...
    *tx = ((*tx * (0x8000 - (max - abs(*tx)) * sb_float_sens)) >> 16) / sb_tran_sens;

    *ty = ((*ty * (0x8000 - (max - abs(*ty)) * sb_float_sens)) >> 16) / sb_tran_sens;

    *tz = ((*tz * (0x8000 - (max - abs(*tz)) * sb_float_sens)) >> 16) / sb_jump_sens;

    *rx = ((*rx * (0x8000 - (max - abs(*rx)) * sb_float_sens)) >> 16) / sb_pitch_sens;

    *ry = ((*ry * (0x8000 - (max - abs(*ry)) * sb_float_sens)) >> 16) / sb_rot_sens;

    *rz = ((*rz * (0x8000 - (max - abs(*rz)) * sb_float_sens)) >> 16) / sb_rot_sens;
}

    // *** WARNING *** WARNING *** WARNING *** WARNING *** WARNING *** WARNING *** WARNING *** WARNING *** WARNING ***
    // WARNING *** These #defines are also used in panels.c - if your change them here be sure to change them there!
...
#endif
    {
        ...
        */
        ...
        if (vals[2] >= 0) {
            ...
            physics_set_player_controls(INP6D_CONTROL_BANK, vals[0], vals[1], vals[2], vals[3], 0, 0);

        }

        else {

            physics_set_player_controls(INP6D_CONTROL_BANK, vals[0], vals[1], 0, vals[3], 0, 0);
...
#ifdef __ANDROID__
static void rp5np_controller_game_action(rp5np_controller_action action, void *userdata) {
    (void)userdata;

    switch (action) {
    case RP5NP_CONTROLLER_ACTION_CROUCH_TOGGLE:
        if (player_struct.posture == POSTURE_STOOP || player_struct.posture == POSTURE_PRONE)
            player_set_posture(POSTURE_STAND);
        else
            player_set_posture(POSTURE_STOOP);
        SDL_Log("RP5NP_REMAKE_ACTION=CROUCH_TOGGLE posture=%u", (unsigned)player_struct.posture);
        break;

    case RP5NP_CONTROLLER_ACTION_SWAP_WEAPON_MODE:
        reload_weapon_hotkey(0, DEMO_CONTEXT, 1);
        SDL_Log("RP5NP_REMAKE_ACTION=SWAP_WEAPON_MODE");
        break;

    case RP5NP_CONTROLLER_ACTION_QUICK_CONSUMABLE:
        use_drug_hotkey(0, DEMO_CONTEXT, 0);
        SDL_Log("RP5NP_REMAKE_ACTION=QUICK_CONSUMABLE");
        break;

    case RP5NP_CONTROLLER_ACTION_QUICK_EXPLOSIVE:
        arm_grenade_hotkey(0, DEMO_CONTEXT, 0);
        SDL_Log("RP5NP_REMAKE_ACTION=QUICK_EXPLOSIVE");
        break;

    case RP5NP_CONTROLLER_ACTION_TARGET_ID:
        use_ware(WARE_HARD, HARDWARE_TARGET);
        SDL_Log("RP5NP_REMAKE_ACTION=TARGET_ID");
        break;

    case RP5NP_CONTROLLER_ACTION_CYCLE_WEAPON_PREV:
        cycle_weapons_func(0, DEMO_CONTEXT, -1);
        SDL_Log("RP5NP_REMAKE_ACTION=CYCLE_WEAPON_PREV");
        break;

    case RP5NP_CONTROLLER_ACTION_CYCLE_WEAPON_NEXT:
        cycle_weapons_func(0, DEMO_CONTEXT, 1);
        SDL_Log("RP5NP_REMAKE_ACTION=CYCLE_WEAPON_NEXT");
        break;

    case RP5NP_CONTROLLER_ACTION_LOOK_MODE: {
        extern int mlook_enabled;
        if (!mlook_enabled)
            mouse_look_toggle();
        SDL_Log("RP5NP_REMAKE_ACTION=LOOK_MODE state=%s", mlook_enabled ? "ON" : "OFF");
    } break;

    case RP5NP_CONTROLLER_ACTION_CURSOR_MODE: {
        extern int mlook_enabled;
        if (mlook_enabled)
            mouse_look_toggle();
        SDL_Log("RP5NP_REMAKE_ACTION=CURSOR_MODE state=%s", mlook_enabled ? "LOOK" : "CURSOR");
    } break;
...
#ifdef __ANDROID__
    /* High-level System Shock actions stay in GameSrc. The SDL controller
       adapter emits abstract remake-style actions through this registered
       callback instead of importing game/UI headers into INPUT_LIB. */
    rp5np_set_controller_action_handler(rp5np_controller_game_action, NULL);
    SDL_Log("RP5NP_CONTROLLER_ACTION_HANDLER=READY profile=RP5_REMAKE_V1");
#endif
...
void inp6d_chk(void) {
    ...
    if (inp6d_dbg)
        mprintf("Parsed %04x %04x %04x %04x %04x %04x to %d %d %d %d %d %d\n", inp6d_in->x, inp6d_in->y, inp6d_in->z,
                inp6d_in->rx, inp6d_in->ry, inp6d_in->rz, xp, yp, zv, h, p, b);
#endif
...
short *set_abs_head(i6s_event *e) {
    ...
    if (inp6d_hdouble)
        l_angs[ANG_H] += l_angs[ANG_H];
    if (inp6d_pdouble)
        l_angs[ANG_P] += l_angs[ANG_P];
    if (inp6d_bdouble)
        l_angs[ANG_B] += l_angs[ANG_B];
    return l_angs;
}
...
void inp_weapon_button(uchar pull) {
    ...
    if ((player_struct.weapons[w].type != GUN_SUBCLASS_BEAM) &&
        ...
        (player_struct.weapons[w].type != GUN_SUBCLASS_BEAMPROJ) && (player_struct.weapons[w].ammo == 0)) { // reload
        ...
        LGPoint pos =
            MakePoint(_current_view->abs_x + RectWidth(_current_view->r) / 2,
...
void vfx1_chk(void) {
    ...
    if (abs(yp) > VFX1_TRA_TOL)
        if (yp > VFX1_TRA_TOL)
            yp = CONTROL_MAX_VAL / 4 + VFX1_Tra_Scale(yp - VFX1_TRA_TOL);
        else
            yp = VFX1_Tra_Scale(yp + VFX1_TRA_TOL);
    else
        yp = 0;
    xp1 = xp;
    xp2 = 0;
    ...
    if (inp_sidestep) {
        xp2 = xp1 * 2;
        xp1 = 0;
    } // switch bat X from heading to sidestep
    ...
    if (inp6d_dbg)
        mprintf("Parsed %04x %04x %04x %04x %04x %04x to %d %d %d %d %d %d\n", inp6d_in->x, inp6d_in->y, inp6d_in->z,
                inp6d_in->rx, inp6d_in->ry, inp6d_in->rz, xp, yp, zv, angs[ANG_P], angs[ANG_B], angs[ANG_H]);
#endif
...
#endif // PLAYTEST
    {
        ...
        if (global_fullmap->cyber) {
            ...
            if (xp1 > CONTROL_MAX_VAL)
                xp1 = CONTROL_MAX_VAL;
            else if (xp1 < -CONTROL_MAX_VAL)
                xp1 = -CONTROL_MAX_VAL;

            if (yp > CONTROL_MAX_VAL)
                yp = CONTROL_MAX_VAL;
            else if (yp < -CONTROL_MAX_VAL)
                yp = -CONTROL_MAX_VAL;

            if (xp2 > CONTROL_MAX_VAL)
                xp2 = CONTROL_MAX_VAL;
            else if (xp2 < -CONTROL_MAX_VAL)
                xp2 = -CONTROL_MAX_VAL;
...
void swift_chk(void) {
    ...
    if (inp6d_dbg)
        mprintf("Parsed %04x %04x %04x %04x %04x %04x to %d %d %d %d %d %d\n", inp6d_in->els[0], inp6d_in->els[1],
                inp6d_in->els[2], inp6d_in->els[3], inp6d_in->els[4], inp6d_in->els[5], our_vals[0], our_vals[1],
                our_vals[2], our_vals[3], our_vals[4], our_vals[5]);
#endif
...
#endif
    {
        physics_set_player_controls(INP6D_CONTROL_BANK, our_vals[0], our_vals[1], our_vals[2], our_vals[3], our_vals[4],
                                    our_vals[5]);
    }
}
...
uchar change_gamma(ushort keycode, uint32_t context, intptr_t data) {
    ...
    if ((dir < 0) && (cit_gamma > fix_make(0, 0x6000)))
        cit_gamma -= fix_make(0, 0x0400);
    else if ((dir > 0) && (cit_gamma < fix_make(1, 0x6000)))
        cit_gamma += fix_make(0, 0x0800);
    else
        dir = 0;
    if (dir != 0)
        gr_set_gamma_pal(0, 256, cit_gamma);
    // here, should do message_info ResXXX+1-dir, have 3 strings in file, Lowered, Maxed, Raised
    // instead, ill do a switch for now...
    switch (dir) {
    case -1:
        message_info("Gamma Lowered");
        break;
    case 0:
        message_info("Gamma Maxed Out");
        break;
    case 1:
        message_info("Gamma Raised");
        break;
    }
    return FALSE;
}
...
void cyberman_chk(void) {
    ...
    if (inp6d_dbg)
        mprintf("Parsed %04x %04x %04x %04x %04x %04x b%x to %d %d %d %d %d %d\n", inp6d_in->x, inp6d_in->y,
                inp6d_in->z, inp6d_in->rx, inp6d_in->ry, inp6d_in->rz, inp6d_in->but, xp, yp, zv, h, p, b);
#endif
...
#endif
    {
        ...
        if (p > 0) {
            if (p_vel > 0)
                ...
                p_vel = 16;
            if (p_vel > CONTROL_MAX_VAL)
                p_vel = CONTROL_MAX_VAL;
        } else if (p < 0) {
            if (p_vel < 0)
                p_vel -= 6;
            else
...
void joystick_chk(void) {
    ...
    if (abs(h) > JOY_TOL)
        if (h > JOY_TOL)
            h = JOY_Scale(h - JOY_TOL);
        else if (h < -JOY_TOL)
            h = JOY_Scale(h + JOY_TOL);
        else
            h = 0;

    if (h > CONTROL_MAX_VAL)
        h = CONTROL_MAX_VAL;
    else if (h < -CONTROL_MAX_VAL)
        h = -CONTROL_MAX_VAL;
    if (yp > CONTROL_MAX_VAL)
        yp = CONTROL_MAX_VAL;
    else if (yp < -CONTROL_MAX_VAL)
    ...
    if (inp6d_dbg) {
        if (bstate != 0)
            mprintf("Yo buttons %2.2x..", bstate);
        else
            mprintf("               ");
