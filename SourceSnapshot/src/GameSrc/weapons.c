...
uchar weapon_energy_drain(weapon_slot *ws, ubyte charge, ubyte max_charge);
uchar player_fire_energy(LGPoint *pos, ubyte slot, int gun_triple);
...
uchar player_fire_handtohand(LGPoint *p, ubyte slot, ObjID *what_hit, int gun_triple) {
    ...
    *what_hit = OBJ_NULL;
    ...
    view_pt.x = new_state.X;
    view_pt.y = new_state.Y;
    view_pt.z = new_state.Z;

    // get the body point of the player
    EDMS_get_state(objs[PLAYER_OBJ].info.ph, &new_state);
    body_pt.x = new_state.X;
    body_pt.y = new_state.Y;
    body_pt.z = new_state.Z;
    ...
    for (i = (HITLOCS - 1); i >= 0; i--) {
        ...
        target = ray_cast_vector(PLAYER_OBJ, &origin, vector, NO_KNOCKBACK_MASS, RAYCAST_ATTACK_SIZE,
    ...
    if (hit_wall || (target != OBJ_NULL)) {
        ...
        ray_cast_vector(PLAYER_OBJ, &origin, vector, attack_mass, RAYCAST_ATTACK_SIZE,
...
uchar player_fire_projectile(LGPoint *pos, LGRegion *r, ubyte slot, int gun_triple) {
    ...
    get_phys_state(objs[PLAYER_OBJ].info.ph, &new_state, PLAYER_OBJ);
    origin.x = new_state.X;
    origin.y = new_state.Y;
    origin.z = new_state.Z;

    find_fire_vector(pos, &vector);
    bullet_mass = fix_make(AmmoProps[CPTRIP(ammo_triple)].bullet_mass, 0) * 30;

    target = ray_cast_vector(PLAYER_OBJ, &origin, vector, bullet_mass, RAYCAST_ATTACK_SIZE,
...
uchar player_fire_slow_projectile_weapon(LGPoint *pos, ubyte slot, int gun_triple) {
    ...
    return (player_fire_slow_projectile(SpecialGunProps[SCTRIP(gun_triple)].proj_triple, gun_triple,
                                        ...
                                        fix_make(SpecialGunProps[SCTRIP(gun_triple)].attack_speed, 0), proj_speed,
                                        pos));
}
...
uchar player_fire_slow_projectile(int proj_triple, int fire_triple, fix proj_mass, fix fire_spd, ubyte proj_speed,
    ...
    if (global_fullmap->cyber) {
        ...
        new_pos.x -=
            (((fauxrend_context *)_current_fr_context)->xtop + (((fauxrend_context *)_current_fr_context)->xwid >> 1));
        new_pos.y -=
            (((fauxrend_context *)_current_fr_context)->ytop + (((fauxrend_context *)_current_fr_context)->ywid >> 1));
    ...
    if (fire_spd && !global_fullmap->cyber) {
        ...
        origin.y = new_state.Y;
        origin.z = new_state.Z;
        ray_cast_vector(PLAYER_OBJ, &origin, vector, proj_mass, RAYCAST_ATTACK_SIZE, fire_spd, PROJ_RAYCAST_RANGE);
    }
    ...
    if (global_fullmap->cyber) {
        new_pos.x =
            (((fauxrend_context *)_current_fr_context)->xwid >> 1) + ((fauxrend_context *)_current_fr_context)->xtop;
        new_pos.y =
            (((fauxrend_context *)_current_fr_context)->ywid >> 1) + ((fauxrend_context *)_current_fr_context)->ytop;
    ...
    //   Spew(DSRC_PHYSICS_Collisions, ("We are ignoring collisions between ph: %d and ph:
...
uchar player_fire_energy_proj(LGPoint *pos, ubyte slot, int gun_triple) {
    ...
    player_fire_slow_projectile(BeamprojGunProps[subclass].proj_triple, MAKETRIP(charge, 0, ws->subtype),
                                (fix_make(BeamprojGunProps[subclass].attack_mass, 0) * 20),
                                fix_make(BeamprojGunProps[subclass].attack_speed, 0),
                                BeamprojGunProps[SCTRIP(gun_triple)].speed, pos);
...
uchar fire_player_software(LGPoint *pos, LGRegion *reg, uchar pull) {
    ...
    case SOFTWARE_SPEW:
        retval =
            player_fire_slow_projectile(cspace_slow_projs[soft], player_struct.softs.combat[soft], 0, FIX_UNIT, 3, pos);
        break;
    }
    return retval;
}
...
char weapon_combat_fxs[] = {
    SFX_GUN_MINIPISTOL, SFX_GUN_DARTPISTOL, SFX_GUN_MAGNUM,         SFX_GUN_ASSAULT,
    SFX_GUN_RIOT,       SFX_GUN_FLECHETTE,  SFX_GUN_SKORPION,       SFX_GUN_MAGPULSE,
    SFX_GUN_RAILGUN,    SFX_GUN_PIPE_MISS,  SFX_GUN_LASEREPEE_MISS, SFX_GUN_PHASER,
    SFX_GUN_BLASTER,    SFX_GUN_IONBEAM,    SFX_GUN_STUNGUN,        SFX_GUN_PLASMA,
};
...
uchar fire_player_weapon(LGPoint *pos, LGRegion *r, uchar pull) {
    ...
    if ((player_struct.weapons[w].type != GUN_SUBCLASS_BEAM) &&
        ...
        (player_struct.weapons[w].type != GUN_SUBCLASS_BEAMPROJ) && (player_struct.weapons[w].ammo == 0)) {
        weapon_mfd_for_reload();
        return FALSE;
    }
    ...
    if (player_struct.weapons[w].type == GUN_SUBCLASS_AUTO) {
        if (!pull) {
            ulong deltat =
                (player_struct.game_time - player_struct.last_fire) * player_struct.fire_rate + weapon_fire_remainder;
    ...
    if (handart_ok) {
        ...
        case BATON_TRIPLE:
            if (hit_obj != OBJ_NULL) {
                if ((objs[hit_obj].obclass == CLASS_CRITTER) && (objs[hit_obj].subclass == CRITTER_SUBCLASS_MUTANT))
...
void get_available_ammo_type(int guntype, int gun_subtype, int *num_ammo_types, ubyte *bitflag, int *ammo_subclass) {
    ...
    for (i = 0; i < 3; i++, type >>= 1) {
        if (type & 0x01) {
            ...
            if (player_struct.cartridges[CPTRIP(triple)] != 0 || player_struct.partial_clip[CPTRIP(triple)] > 0)
                bitflag[count++] = i;
        }
    }
...
void check_temperature(weapon_slot *ws, uchar clear) {
    ...
    if (!OVERLOAD_VALUE(ws->setting) && ((ws->heat + (ws->setting / 2)) > (OVERHEAT_THRESHOLD + HEAT_FUDGE))) {
        if (!temp_critical)
            hud_set(HUD_BEAMHOT);
        temp_critical = TRUE;
    } else {
...
void cool_off_beam_weapons(void) {
    ...
    while (running_dt > WEAPON_COOL_OFF_TIME) {
        running_dt =
            lg_max(0, running_dt - WEAPON_COOL_OFF_TIME); // must be non frame-rate dependant, even if goofily so.
