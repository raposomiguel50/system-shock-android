...
void _fr_draw_parm_cube(grs_bitmap *side_bm, grs_bitmap *oth_bm, int x, int y, int z) {
    ...
    x += x;
    y += y;
    z = -(z + z); // go from radius to real
    cube_pt[1] = g3_copy_add_delta_z(cube_pt[0], y);
    cube_pt[1]->p3_flags |= PF_U | PF_V;
    cube_pt[2] = g3_copy_add_delta_x(cube_pt[1], x);
    cube_pt[2]->p3_flags |= PF_U | PF_V;
    cube_pt[3] = g3_copy_add_delta_x(cube_pt[0], x);
    cube_pt[3]->p3_flags |= PF_U | PF_V;
    cube_pt[4] = g3_copy_add_delta_y(cube_pt[0], z);
    cube_pt[4]->p3_flags |= PF_U | PF_V;
    cube_pt[5] = g3_copy_add_delta_y(cube_pt[1], z);
    cube_pt[5]->p3_flags |= PF_U | PF_V;
    cube_pt[6] = g3_copy_add_delta_y(cube_pt[2], z);
...
#define fpoly_rend(pc, fc, fpts)                    \
    if (pc == 0)                                    \
        pc = -0xff;                                 \
    if (pc >= 0)                                    \
        g3_check_and_draw_poly(pc, fc, fpts);       \
    else if (pc < -0x80)                            \
        g3_check_and_draw_tluc_poly(-pc, fc, fpts); \
    else                                            \
        g3_check_and_draw_tluc_spoly(fc, fpts)
...
void _fr_draw_poly_cube(int p_color, int x, int y, int z) {
    ...
    if (haktype == 0)
        y = (y * (haky + 1)) >> 8;
    cube_pt[1] = g3_copy_add_delta_z(cube_pt[0], y);
    cube_pt[1]->p3_flags |= PF_I;
    cube_pt[1]->i = 0x0200;
    cube_pt[2] = g3_copy_add_delta_x(cube_pt[1], x);
    cube_pt[2]->p3_flags |= PF_I;
    cube_pt[2]->i = 0x0300;
    cube_pt[3] = g3_copy_add_delta_x(cube_pt[0], x);
    cube_pt[3]->p3_flags |= PF_I;
    cube_pt[3]->i = 0x0400;
    cube_pt[4] = g3_copy_add_delta_y(cube_pt[0], z);
    cube_pt[4]->p3_flags |= PF_I;
    cube_pt[4]->i = 0x0500;
    cube_pt[5] = g3_copy_add_delta_y(cube_pt[1], z);
    cube_pt[5]->p3_flags |= PF_I;
    cube_pt[5]->i = 0x0600;
    cube_pt[6] = g3_copy_add_delta_y(cube_pt[2], z);
    cube_pt[6]->p3_flags |= PF_I;
    cube_pt[6]->i = 0x0700;
    cube_pt[7] = g3_copy_add_delta_y(cube_pt[3], z);
...
void gen_seed_vec(g3s_vector *gpt_vec, int seed, int scale, int deviant) {
    deviant = (1 << deviant) - 1;
    if (seed != 0) {
        gpt_vec->gX = ((seed & 0x1f) - 0xf);
        seed >>= 5;
        gpt_vec->gY = ((seed & 0x1f) - 0xf);
        seed >>= 5;
...
void do_xplodamatron(int frame, int severity, int seed, int col1, int col2) {
    ...
    for (; severity >= 0; severity--, seed += seed_add) {
        ...
        xplo_vec.gX += 0x4000 + (frame << 11);
        xplo_vec.gY += 0x6000 + (frame << 12);
        xplo_vec.gZ -= 0x3000 + (frame << 11);
        xplo_pts[1] = g3_transform_point(&xplo_vec);
        xplo_vec.gX -= 0x7000 + (frame << 13);
        xplo_vec.gY += 0x5000 + (frame << 10);
        xplo_vec.gZ -= 0x2000 + (frame << 12);
...
void gen_tetra(g3s_phandle *xplo_pts, fix size, int ii, int color) {
    ...
    for (i = 0; i < 4; i++)
        if (color > 0) {
            xplo_pts[i]->rgb = grd_bpal[color];
            xplo_pts[i]->p3_flags |= PF_RGB;
        } else {
            xplo_pts[i]->rgb = grd_bpal[-color + (rand() & 3)];
            xplo_pts[i]->p3_flags |= PF_RGB;
        }
}
...
void draw_ice(void) {
    ...
    p = _fr_cobj->loc.p << 8;
    p += (rand() & (1 << (deviant + 1))) - (1 << deviant);
    h = _fr_cobj->loc.h << 8;
    h += (rand() & (1 << (deviant + 1))) - (1 << deviant);
    b = _fr_cobj->loc.b << 8;
    b += (rand() & (1 << (deviant + 1))) - (1 << deviant);
...
void _fr_draw_tmtile(grs_bitmap *draw_bm, int col_val, g3s_phandle *plst, uchar dblface, uchar use_lighting) {
    ...
    _fdt_pbase = t_off_l;
    _fr_do_light(plst[0], FRPTSZCEIL_DN);
    plst[0]->uv.u = 0;
    plst[0]->uv.v = 0;
    plst[0]->p3_flags = PF_I | PF_V | PF_U;
    _fr_do_light(plst[3], FRPTSZFLR_DN);
    plst[3]->uv.u = 0;
    plst[3]->uv.v = 0x100;
    plst[3]->p3_flags = PF_I | PF_V | PF_U;
    _fdt_pbase = t_off_r;
    _fr_do_light(plst[1], FRPTSZCEIL_DN);
    plst[1]->uv.u = 0x100;
    plst[1]->uv.v = 0;
    plst[1]->p3_flags = PF_I | PF_V | PF_U;
    _fr_do_light(plst[2], FRPTSZFLR_DN);
    plst[2]->uv.u = 0x100;
    plst[2]->uv.v = 0x100;
    plst[2]->p3_flags = PF_I | PF_V | PF_U;
    if (!use_lighting) {
        plst[0]->i = 0;
        plst[3]->i = 0;
        plst[1]->i = 0;
    ...
    if (dblface) // draw the bleeding backside, nudge nudge
    {
        ...
        tmp = plst[2];
        plst[2] = plst[3];
        plst[3] = tmp; // and if you thought that was obscure
        if (_fr_curflags & FR_PICKUPM_MASK)
            if ((draw_bm == NULL) || (_fr_curflags & FR_NOTRANS_MASK) || ((draw_bm->flags & BMF_TRANS) == 0))
                g3_check_and_draw_poly(gr_get_fill_parm(), 4, plst);
            else
...
void _fr_draw_bitmap(grs_bitmap *draw_bm, int dist, int sc, int anch_x, int anch_y) {
    ...
    if (sc)
        g3_set_bitmap_scale(fix_make(0, (int)(4096 / 3)), fix_make(0, (int)(4096 / 3)));
    ...
    if ((bitmap_verts != NULL) && IS_HUDOBJ(_fr_cobj - objs)) {
        ...
        for (i = 0; i < 4; i++) {
            if (bitmap_verts[i]->x < lx)
                lx = bitmap_verts[i]->x;
            if (bitmap_verts[i]->x > rx)
                rx = bitmap_verts[i]->x;
            if (bitmap_verts[i]->y < ly)
                ly = bitmap_verts[i]->y;
...
//   mprintf("cobjid = %x\n",cobjid);
    ...
    if (global_fullmap->cyber) {
        if ((obj_type != FAUBJ_CRIT) && (obj_type != FAUBJ_TEXTPOLY) && (obj_type != FAUBJ_VOX) &&
            (obj_type != FAUBJ_SPECIAL)) {
            ...
            case CLASS_HARDWARE:
                col = 0x48 - (objHardwares[_fr_cobj->specID].version << 1);
                if (col < 0x45)
                    col = 0x42;
                break;
            case CLASS_BIGSTUFF:
            ...
            if (dm == 0) {
                ...
                if (dm & 0x40)
                    ...
                    fz = bsc;
                if ((csc == 0x4500) && (dm & 0x8))
                    ndm = 0x7;
                else if ((csc == 0x1000) && ((dm & 0x8) == 0))
                    ndm = 0x8;
                else
                    ndm = 0;
                if (ndm) {
                    dm = (dm & ~0xf) + ndm;
                    if (((dm & 0x88) == 0x88) && ((rand() & 0x7) == 0))
                        dm = (dm & ~0x70) + (rand() & 0x70);
                }
                if (dm & 0x8)
                    dm++;
                else
...
// when I have a more coherent plan WRT it.
        ...
        if (((ObjProps[objtrip].flags & LIGHT_TYPE) <= (1 << LIGHT_TYPE_SHF)) ||
            (((ObjProps[objtrip].flags & LIGHT_TYPE) == (3 << LIGHT_TYPE_SHF)) &&
             ((_fr_cobj->info.inst_flags & UNLIT_FLAG) == 0))) {
        MapElem *tmp;
        int _obj_do_light(int which, fix dist);
        fix hack_dist_approx =
            fix_fast_pyth_dist(fix_fast_pyth_dist((_fr_p.gX - fr_camera_last[0]), (_fr_p.gZ - fr_camera_last[1])),
                               (_fr_p.gY + fr_camera_last[2]));
        tmp = _fdt_mptr;
        _fdt_pbase = 0;
    ...
    case FAUBJ_BITMAP:
        if ((anchors_3d[o3drep].x > 0) || (anchors_3d[o3drep].y > 0))
            _o_rad = 0;
        _fr_draw_bitmap(bitmaps_3d[o3drep], _fdt_dist, ObjProps[objtrip].flags & MY_IM_LARGE, anchors_3d[o3drep].x,
                        anchors_3d[o3drep].y);
        if (global_fullmap->cyber)
            if (obj_ICE_ICE_BABY(_fr_cobj))
                draw_ice();
    ...
    case FAUBJ_VOX: { // wait, cant we just keep vvv around, and stuff the two bitmaps, w+h each time, save some here
        vxs_vox vvv;
    ...
    case FAUBJ_TPOLY: {
        ...
        if ((_fr_p.gX & 0xffff) == 0xff00)
            _fr_p.gX += 0x100;
        if ((_fr_p.gZ & 0xffff) == 0xff00)
            _fr_p.gZ += 0x100;
        ...
        if (global_fullmap->cyber) {
            ...
            case CLASS_SMALLSTUFF:
                ...
                for (foog = 0; foog < NUM_SMALLSTUFF_VCOLORS; foog++) {
                    ...
                    //                  Warning(("setting vcolor %d to
                    //                  0x%x!\n",foog+1,CyberSmallstuffProps[SCNUM(cobjid)].vcolors[foog]));
                }
                //                  g3_set_vcolor(foog+1,0x33 + objSmallstuffs[_fr_cobj->specID].cosmetic_value + (foog
                //                  << 2));
                break;
            case CLASS_PHYSICS:
