...
errtype init_hack_cameras() {
    ...
    fr_camera_create(&hack_cam, CAMTYPE_OBJ, player_struct.rep, NULL, NULL);
    for (i = 0; i < MAX_CAMERAS_VISIBLE; i++) {
        ...
        hack_cam_frcs[i] = fr_place_view(FR_NEWVIEW, &hack_cam, tmp_mem, FR_DOUBLEB_MASK | FR_HACKCAM_FLAG, 0, 0,
                                         HACK_CAMERA_X, HACK_CAMERA_Y, HACK_CAMERA_WIDTH, HACK_CAMERA_HEIGHT);
...
errtype render_hack_cameras() {
    ...
    for (i = 0; i < NUM_HACK_CAMERAS; i++) {
        if (camera_map[i] && (hack_frc_objs[camera_map[i] - 1] != hack_cam_objs[i] ||
