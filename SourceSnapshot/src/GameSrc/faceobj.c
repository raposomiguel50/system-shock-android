...
uchar _axial_relativize(fix *src_pts, fix *targ_pts, uchar vec) // (fixang l_h, fixang l_p, fixang l_b)
{
    ...
    // now go relativize
    if ((l_p) | (l_b)) { // if any pitch of bank
        if (((l_b) | (l_h)) == 0)
            _1d_relativize(l_p, src_pts, targ_pts, 1, 2, 0);
        else if ((l_b) == 0) { // 2d transform, just pitch and heading..
                               //         Warning(("Obj with pitch and heading changed\n"));
            return FALSE;
        } else { // unsupported case for now
...
uchar setup_cube_face(fix ndist, fix xp, fix yp, fix xhlf, fix yhlf, int nrm_cmp) {
    ...
    //   mprintf("SCF: dist %x, pos %x %x, size %x %x, norm %x %x %x f
...
void _face_secret_repulsor_hack(void) {
    ...
    if (tf_ph == -1)
        return;                                     // dont fuck with bullets or L-O-Sight
    if ((tf_loc_pt[0] < 0) || (tf_loc_pt[1] < 0) || // if we are not in the main square for
        (tf_loc_pt[0] >= fix_1) || (tf_loc_pt[1] >= fix_1))
        return; //   the object, dont repulse it..

    if (!comparator_check(comparator, OBJ_NULL, &special))
        return; // make sure the repulsor is active

    if ((r_bot = objTraps[_n_fr_cobj->specID].p2) != 0) // if lower bound
        if (objTraps[_n_fr_cobj->specID].p2 > tf_raw_pt[2])
            return;                                     // too low
    if ((r_top = objTraps[_n_fr_cobj->specID].p3) != 0) // if upper bound
        if (r_top + (tf_rad >> 1) + (tf_rad >> 2) < tf_raw_pt[2])
            return; // too high
    flg = (objTraps[_n_fr_cobj->specID].p4 + 1) << SS_BCD_REPUL_SHF;
    if ((flg & SS_BCD_REPUL_TYPE) == SS_BCD_REPUL_UP) {
        if (r_top == 0)
            r_prm = fix_make(7453, 0); // that should be pretty darn high, folks
        else
            r_prm = r_top + (tf_rad >> 1) + (tf_rad >> 2);
    } else
        r_prm = r_bot;
...
void facelet_obj(ObjID cobjid) {
    ...
    case FAUBJ_TPOLY: {
        ...
        fix_xoff = fix_make(0, 0x0200) << 6;
        fix_yoff = fix_make(0, 0x0200) << 6;
        if (scale > 0) {
            fix_xoff <<= scale;
            fix_yoff <<= scale;
        } else {
