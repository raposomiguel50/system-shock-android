...
#define SetAutomapMode(m, mode) \
    player_struct.mfd_func_data[MFD_MAP_FUNC][(m)] = (player_struct.mfd_func_data[MFD_MAP_FUNC][(m)] & 0xfc) + (mode)
#define GetAutomapMode(m) (player_struct.mfd_func_data[MFD_MAP_FUNC][(m)] & 0x03)
...
#define GetLastAutomapMode(m) ((player_struct.mfd_func_data[MFD_MAP_FUNC][(m)] & 0x0c) >> 2)
...
#define WORKING_INC_MFDS
...
#endif
                {
                    if ((AutomapLastUpdated == mapid) && (GetAutomapMode((mapid == 0) ? 1 : 0) ==
                                                          AUTOMAP_ZOOM)) { // we were last done, and both of us need to
                                                                           // be done, so give the other guy a chance...
        ...
        case AUTOMAP_ZOOM:
            ...
            break;
        case AUTOMAP_STATION:
            automap_expose_cross_section(m, control);
            break;
        }
        gr_pop_canvas();
        mfd_update_display(m, 0, 0, MFD_VIEW_WID, MFD_VIEW_HGT);
