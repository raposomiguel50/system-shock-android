...
int get_ware_triple(int waretype, int num) { return nth_after_triple(ware_base_triples[waretype], num); }
...
void use_ware(int waretype, int num) {
    ...
    if (!WareActive(player_status[num])) { // we're turning a ware off
        ...
        if (wares[num].turnoff)
            wares[num].turnoff(TRUE, TRUE);
        switch (num) {
        case HARDWARE_360:
...
void bioware_effect(void) { mfd_notify_func(MFD_BIOWARE_FUNC, MFD_INFO_SLOT, FALSE, MFD_ACTIVE, FALSE); }
...
void lamp_set_vals_with_offset(byte offset) {
    ...
    _frp.lighting.base[0] =
        (uchar)((lspec->base1 + (offset >> OFF_SHF) > 0) ? (lspec->base1 + (offset >> OFF_SHF)) : 0);
    if (offset != 0) {
        fix slope_based_mod;
...
void motionware_turnon(uchar visible, uchar real) { motionware_update(visible, real, TRUE); }
