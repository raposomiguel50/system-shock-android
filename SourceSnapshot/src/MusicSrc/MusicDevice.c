...
static void AdlMidiGenerate(MusicDevice *dev, short *samples, int numframes)
{
    ...
    // RP5 audio fidelity pass: preserve the established 2x ADLMIDI gain,
    // but saturate in a wider integer domain instead of overflowing int16.
    short *sample = samples;
    for (int i = 0; i < numSamples; ++i, ++sample)
    {
        const int amplified = (int)(*sample) * 2;
        if (amplified > 32767)
            *sample = 32767;
