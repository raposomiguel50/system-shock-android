...
TerrainHit indoor_terrain(Q X, Q Y, Q Z, Q R, physics_handle ph, TFType type) {
    if ((X > 1) && (Y > 1) && (X < 64) && (Y < 64)) {
        ...
        if (ph > -1) {
            int on = ph2on[ph];
            //         mout << "!EDMS: Integration location (should match): (" << A[on][0][0] << ", " << A[on][1][0] <<
            //         ", " << A[on][2][0] << ").\n"; mout << "!EDMS: Object location last frame: (" << S[on][0][0] <<
            //         ", " << S[on][1][0] << ", " << S[on][2][0] << ").\n"; mout << "!EDMS: Sleep: " <<
            //         no_no_not_me[on] << ", EDMS_sanity_check = " << sanity_check() << ".\n"; mout << "!EDMS: object
            //         number " << on << ", EDMS_type: " << I[on][30] << "\n"; mout << "!EDMS: Calling AWOL
...
void EDMS_kill_object(physics_handle ph) {
    ...
    // ========================
    if (ph > -1) {
        int on = physics_handle_to_object_number(ph), i;
...
physics_handle EDMS_get_free_ph(void) {
    for (int32_t i = min_physics_handle; i < MAX_OBJ; ++i) {
        // Ho, ho...
        // ---------
        if (ph2on[i] == -1)
            return i;
