...
/* RP5 native Android: the legacy scheduler used SDL_GetTicks() and would
 * immediately drain overdue MIDI/XMI events after a host scheduling stall.
 * On Android that catch-up burst is audible as transient fast/out-of-sync
 * gameplay music. Use the monotonic high-resolution performance counter and
 * shift every active music-thread clock together when lateness exceeds a
 * conservative threshold. Shifting all active clocks preserves dynamic layer
 * phase instead of rebasing one layer independently. */
#define RP5NP_XMI_REBASE_THRESHOLD_US 30000.0

static double rp5np_xmi_clock_us(void) {
    const Uint64 frequency = SDL_GetPerformanceFrequency();
    if (frequency == 0)
        return (double)SDL_GetTicks64() * 1000.0;
    return ((double)SDL_GetPerformanceCounter() * 1000000.0) / (double)frequency;
}
...
int MyThread(void *arg) {
    ...
    double start_us[NUM_THREADS];
    unsigned int rebase_count[NUM_THREADS];
    ...
    SDL_Log("RP5NP_XMI_CLOCK_POLICY=PERF_COUNTER_GLOBAL_LATE_REBASE threshold_us=%.0f",
            RP5NP_XMI_REBASE_THRESHOLD_US);
    ...
    for (;;) {
        ...
        for (i = 0; i < NUM_THREADS; i++) {
            if (event[i] && SDL_AtomicGet(&ThreadCommand[i]) != THREAD_STOPTRACK) {
                const double now_us = rp5np_xmi_clock_us();
                double aim = last_time[i] + (event[i]->time - last_tick[i]) * tick[i];
                double diff = aim - (now_us - start_us[i]);

                if (diff < -RP5NP_XMI_REBASE_THRESHOLD_US) {
                    const double late_us = -diff;
                    int active_threads = 0;

                    /* One host-thread stall affects all layers. Advance every
                     * active layer's origin by the same amount so the next
                     * musical intervals remain intact and layers stay aligned. */
                    for (int j = 0; j < NUM_THREADS; ++j) {
                        if (event[j] && SDL_AtomicGet(&ThreadCommand[j]) != THREAD_STOPTRACK) {
                            start_us[j] += late_us;
                            active_threads++;
                        }
                    }
                    ...
                    rebase_count[i]++;
                    SDL_Log("RP5NP_XMI_CLOCK_REBASE source_thread=%d late_us=%.0f active_threads=%d count=%u",
                            i, late_us, active_threads, rebase_count[i]);
                    diff = 0.0;
                }
            ...
            if (SDL_AtomicGet(&ThreadCommand[i]) == THREAD_STOPTRACK) {
                ...
                last_tick[i] = 0;
                last_time[i] = 0;
                start_us[i] = rp5np_xmi_clock_us();

                /* A2 audio fidelity: a stopped dynamic layer relinquishes its
                 * remapped melodic channels. Explicitly release notes and reset
                 * controller state before the channels can be reused. The shared
                 * percussion channel (9) is intentionally untouched because it is
                 * not owned through ChannelThread[] and may still be used by another
                 * simultaneously playing layer. */
                int released_channels = 0;
                SDL_LockMutex(MyMutex);
                for (channel = 0; channel < 16; channel++) {
                    if (ChannelThread[channel] == i) {
                        if (MusicDev && MusicDev->isOpen) {
                            /* MIDI CC123: release every active note on this channel. */
                            MusicDev->sendControllerChange(MusicDev, channel, 123, 0);
                            /* MIDI CC121: clear sustain/other controllers so no held
                             * note state leaks into the next layer using this channel. */
                            MusicDev->sendControllerChange(MusicDev, channel, 121, 0);
                        }
                        ChannelThread[channel] = -1;
                        if (NumUsedChannels > 0)
                            NumUsedChannels--;
                        else
                            SDL_Log("RP5NP_XMI_CHANNEL_COUNT_UNDERFLOW thread=%d channel=%d", i, channel);
                ...
                if (released_channels > 0)
                    SDL_Log("RP5NP_XMI_CHANNEL_CLEANUP thread=%d channels=%d", i, released_channels);
...
void StartTrack(int thread, unsigned int track) {
    ...
    int can_start = 0;
    int assigned_channels = 0;
    char channel_remap[16];
    char allocated_device_channels[16];

    if (thread < 0 || thread >= NUM_THREADS || track >= NumTracks)
        return;
    ...
    memset(channel_remap, 0, sizeof(channel_remap));
    memset(allocated_device_channels, 0, sizeof(allocated_device_channels));

    /* A3 audio fidelity: channel allocation and release happen on different
     * threads. Protect ChannelThread[] and NumUsedChannels with the same mutex
     * used by the release path so two dynamic transitions cannot observe a
     * half-updated channel map. */
    SDL_LockMutex(MyMutex);
    ...
    if (NumUsedChannels + num <= 16 - 1) {
        ...
        for (trackChannel = 0; trackChannel < 16; trackChannel++) {
            ...
            if (trackChannel != 9 && (TrackUsedChannels[track] & (1 << trackChannel))) {
                ...
                allocated_device_channels[deviceChannel] = 1;
                assigned_channels++;
        ...
        if (can_start && assigned_channels == num) {
            NumUsedChannels += assigned_channels;
            ...
            // Roll back a partial map rather than leaking channels.
            for (deviceChannel = 0; deviceChannel < 16; deviceChannel++) {
                if (allocated_device_channels[deviceChannel]) {
                    ChannelThread[deviceChannel] = -1;
                    SDL_AtomicSet(&DeviceChannelVolume[deviceChannel], 0);
                }
            }
            assigned_channels = 0;
            can_start = 0;
    ...
    if (can_start) {
        SDL_Log("RP5NP_XMI_CHANNEL_ALLOC thread=%d track=%u channels=%d used=%d",
                thread, track, assigned_channels, NumUsedChannels);
        ...
        SDL_Log("RP5NP_XMI_CHANNEL_ALLOC_REJECT thread=%d track=%u need=%d used=%d",
                thread, track, num, NumUsedChannels);
    }
}
...
void StopTheMusic(void) {
    ...
    if (MusicDev) {
        ...
        /* Keep the shared percussion bookkeeping aligned with the game's
         * established full-volume channel baseline after a device reset. */
        SDL_AtomicSet(&DeviceChannelVolume[9], 127);
    }
    SDL_UnlockMutex(MyMutex);
}
...
void InitReadXMI(void) {
    ...
    /* A3 audio fidelity: InitDecXMI() takes MyMutex, so create the mutex before
     * opening/resetting the synthesizer. The legacy order attempted to lock a
     * NULL mutex during first initialization. */
    MyMutex = SDL_CreateMutex();
    if (!MyMutex) {
        ERROR("InitReadXMI(): could not create MIDI mutex: %s", SDL_GetError());
        return;
    }
    ...
    /* A3 audio fidelity: percussion is shared rather than allocated through
     * ChannelThread[]. Give channel 9 the same full-volume baseline used for
     * newly allocated melodic channels, so a global-volume refresh cannot
     * accidentally collapse percussion to zero before its first CC7 event. */
    SDL_AtomicSet(&DeviceChannelVolume[9], 127);
