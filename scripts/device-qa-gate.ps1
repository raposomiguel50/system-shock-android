...
$BaselineBefore = Get-PackageSnapshot -PackageName $BaselinePackage
...
$BaselineAfterInstall = Get-PackageSnapshot -PackageName $BaselinePackage
if ($BaselineBefore.Path -ne $BaselineAfterInstall.Path -or
    ...
    throw 'Baseline package metadata changed after QA installation.'
}

& $Adb shell am force-stop $QaPackage | Out-Null
& $Adb logcat -c
if ($LASTEXITCODE -ne 0) {
    throw 'Unable to clear logcat before QA smoke test.'
}

& $Adb shell am start -n "$QaPackage/com.rp5np.systemshock.ShockolateActivity"
if ($LASTEXITCODE -ne 0) {
    throw 'Unable to launch QA package for smoke test.'
}

Start-Sleep -Seconds $SmokeSeconds

$QaPid = (& $Adb shell pidof $QaPackage 2>$null | Out-String).Trim()
if ($QaPid -notmatch '^\d+(\s+\d+)*$') {
    throw "QA process was not alive after $SmokeSeconds seconds. pidof output: $QaPid"
}
...
if ($FatalLines.Count -gt 0) {
    throw "Fatal QA logcat entries detected:`n$($FatalLines -join "`n")"
}

$BaselineAfterSmoke = Get-PackageSnapshot -PackageName $BaselinePackage
if ($BaselineBefore.Path -ne $BaselineAfterSmoke.Path -or
    ...
    throw 'Baseline package metadata changed during QA smoke test.'
}
